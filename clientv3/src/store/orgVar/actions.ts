...
export const actions: ActionTree<CoreState, RootState> = {  
  ...
  save({commit, state}, model?: OrgVar) : Promise<Model> {
  ...
  delete({commit, state}, model?: OrgVar): Promise<void> {
    return coreActions.delete({commit, state}, model);
  },

  select({commit, state, dispatch}, model: OrgVar): Promise<Model|undefined> {
