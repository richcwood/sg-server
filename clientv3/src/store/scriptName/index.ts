...
export const state: CoreState = {
  ...
  _url: () => `api/v0/${ModelBaseUrlType.script}`,
  _responseFields: () => 'scriptType name',
  _storeName: StoreType.ScriptNameStore.toString(),
  _promiseStore: new PromiseStore()
};

state.storeUtils = new StoreUtils(state);

export const scriptNameStore: Module<CoreState, RootState> = {
    namespaced: true,
    state,
    actions,
    mutations,
