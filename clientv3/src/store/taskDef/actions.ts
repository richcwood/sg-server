...
export const actions: ActionTree<CoreState, RootState> = {  
  ...
  save({commit, state, dispatch}, model?: TaskDef) : Promise<Model> {
  ...
  delete({commit, state}, model?: TaskDef): Promise<void> {
    return coreActions.delete({commit, state}, model);
  },

  select({commit, state}, model: TaskDef): Promise<Model | undefined> {
