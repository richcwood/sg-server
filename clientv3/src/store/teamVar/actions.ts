...
export const actions: ActionTree<CoreState, RootState> = {  
  ...
  save({commit, state, dispatch}, model?: TeamVar) : Promise<Model> {
  ...
  delete({commit, state}, model?: TeamVar): Promise<void> {
    return coreActions.delete({commit, state}, model);
  },

  select({commit, state, dispatch}, model: TeamVar): Promise<Model|undefined> {
