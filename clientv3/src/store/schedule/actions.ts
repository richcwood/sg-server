...
export const actions: ActionTree<CoreState, RootState> = {  
  ...
  save({commit, state, dispatch}, model?: Schedule) : Promise<Model> {
  ...
  delete({commit, state}, model?: Schedule): Promise<void> {
    return coreActions.delete({commit, state}, model);
  },

  select({commit, state, dispatch}, model: Schedule): Promise<Model|undefined> {
