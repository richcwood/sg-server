...
export const actions: ActionTree<CoreState, RootState> = {  
  ...
  save({commit, state, dispatch}, model?: PaymentMethod) : Promise<Model> {
  ...
  delete({commit, state}, model?: PaymentMethod): Promise<void> {
    return coreActions.delete({commit, state}, model);
  },

  select({commit, state, dispatch}, model: PaymentMethod): Promise<Model|undefined> {
