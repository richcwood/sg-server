...
export const state: CoreState = {
  ...
  _url: () => `api/v0/${ModelBaseUrlType.paymentTransaction}`,
  _storeName: StoreType.PaymentTransactionStore.toString(),
  _promiseStore: new PromiseStore()
};

state.storeUtils = new StoreUtils(state);

export const paymentTransactionStore: Module<CoreState, RootState> = {
    namespaced: true,
    state,
    actions,
    mutations,
