...
export const actions: ActionTree<CoreState, RootState> = {  
  ...
  save({commit, state, dispatch}, model?: Invoice) : Promise<Model> {
  ...
  delete({commit, state}, model?: Invoice): Promise<void> {
    return coreActions.delete({commit, state}, model);
  },

  select({commit, state, dispatch}, model: Invoice): Promise<Model|undefined> {
