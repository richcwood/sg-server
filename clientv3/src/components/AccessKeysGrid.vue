 <template>
    <div>
        ...
        <modal name="create-user-access-key-modal" :classes="'round-popup'" :width="600" :height="820">
            <validation-observer ref="createUserAccessKeyValidationObserver">
                <table class="table" width="100%">
                            ...
                            <div style="margin-top: 12px; width: 100%; height: 400px; overflow: scroll">
                                <div v-for="accessRight of accessRights" :key="accessRight.id">
                                    <label class="checkbox">
                                        <input
                                            type="checkbox"
                                            v-model="accessRightSelections[accessRight.rightId]"
                                            :disabled="
                                                userAccessRightsBitset &&
                                                !userAccessRightsBitset.get(accessRight.rightId)
                                            "
                                        />
                    ...
                    <tr class="tr">
                        <td class="td"></td>
                        <td class="td">(Rights are disabled if you don't have access yourself)</td>
                    </tr>
                    <tr class="tr">
                        <td class="td">Expiration</td>
                        <td class="td">
                            <datepicker
                                input-class="input"
                                v-model="newAccessKey.expiration"
                                name="filterDate"
                            ></datepicker>
                        </td>
                    </tr>
                    <tr class="tr">
                        ...
                        <td class="td">
                            ...
                            <button class="button button-spaced is-primary" @click="onCreateCreateUserAccessKey">
                                Create Access Key
                            </button>
                        </td>
                    </tr>
        ...
        <modal
            name="create-access-key-success-modal"
            :classes="'round-popup'"
            :clickToClose="false"
            :width="700"
            :height="800"
        >
            <div style="width: 100%; background-color: white; padding: 40px">
                <div class="notification is-primary">
                    <span style="font-size: 24px; font-weight: 700"> Secret: {{ newAccessKeySecret }} </span>
                </div>
                <div class="notification is-danger" style="font-size: 20px; font-weight: 700">
                    ATTENTION!
                    <br />
                    This is your only chance to copy this secret. Please copy it somewhere safely before closing this
                    dialog.
                </div>
                <div>
                    Create a file named sg.cfg in the directory where your agent lives (sg-agent-launcher) and paste the
                    contents below into the file cfg file.
                    <br />
                    <br />
                    <div>
                        { "SG_ACCESS_KEY_ID": "{{ newAccessKeyId }}", "SG_ACCESS_KEY_SECRET": "{{ newAccessKeySecret }}"
                        }
                    </div>
                </div>
                <br /><br />
        ...
        <table class="table">
            ...
            <tr v-if="accessKeys.length === 0" class="tr">
                <td colspan="6" class="td">No access keys yet.</td>
            </tr>
            ...
            <tr v-for="accessKey of accessKeys" :key="accessKey.id" class="tr">
                ...
                <td class="td">
                    ...
                    <a
                        class="button-spaced"
                        v-if="calculateAccessKeyStatus(accessKey) === AccessKeyStatus.ACTIVE"
                        @click="onMakeInactiveClicked(accessKey)"
                        >make inactive</a
                    >
                    <a
                        class="button-spaced"
                        v-if="calculateAccessKeyStatus(accessKey) === AccessKeyStatus.INACTIVE"
                        @click="onMakeActiveClicked(accessKey)"
                        >make active</a
                    >
                </td>
            </tr>
            ...
            <tr class="tr">
                <td colspan="7">
                    <button class="button" @click="openCreateAccessKeyModal">
                        Create {{ accessKeyType === AccessKeyType.AGENT ? 'Agent' : 'User' }} Access Key
                    </button>
                </td>
            </tr>
        </table>
...
export default class AccessKeysGrid extends Vue {
    ...
    private async onMakeInactiveClicked(accessKey: AccessKey) {
        try {
            ...
            this.$store.dispatch(
                ...
                new SgAlert('Inactivated access key', AlertPlacement.FOOTER, AlertCategory.INFO)
            );
        } catch (err) {
            console.error(err);
    ...
    private onMakeActiveClicked(accessKey: AccessKey) {
        try {
            ...
            this.$store.dispatch(
                `${StoreType.AlertStore}/addAlert`,
                new SgAlert('Activated access key', AlertPlacement.FOOTER, AlertCategory.INFO)
            );
        } catch (err) {
            console.error(err);
    ...
    private onMakeExpireClicked(accessKey: AccessKey) {
        try {
            ...
            this.$store.dispatch(
                `${StoreType.AlertStore}/addAlert`,
                new SgAlert('Expired access key', AlertPlacement.FOOTER, AlertCategory.INFO)
            );
        } catch (err) {
            console.error(err);
    ...
    private async openCreateAccessKeyModal() {
        if (this.accessKeyType === AccessKeyType.USER) {
            ...
            this.$modal.show(
                ...
                {
                    'accessKey:created': ({ accessSecret, accessKey }) => {
                        this.newAccessKeySecret = accessSecret;
                        this.newAccessKeyId = accessKey;
    ...
    private async onCreateCreateUserAccessKey() {
        ...
        try {
            ...
            newAccessKey.accessRightIds = Object.keys(this.accessRightSelections).reduce(
                (accum: any, accessKey: string) => {
            ...
            this.$store.dispatch(
                `${StoreType.AlertStore}/addAlert`,
                new SgAlert('Created new user access key', AlertPlacement.FOOTER, AlertCategory.INFO)
            );
    ...
    private closeAccessKeySuccessModal() {
        ...
        if (this.newAccessKeyId) {
            this.$store.commit(`${StoreType.AccessKeyStore}/update`, {
                id: this.newAccessKeyId,
