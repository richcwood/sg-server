        ...
        <template #no>
            <section class="columns content">
                <div class="column is-narrow">
                    <div class="box">
                        <p>Let's run a script right now on AWS Lambda with SaasGlue. First click what language you want
                            the
                            script to be in.</p>
                        <div class="select is-fullwidth" :class="{ 'is-loading': creatingScript }">
                            <select @change="onScriptSelect" v-model="selectedScriptType">
                                <option v-for="(value, key) in scriptTypes" :key="`scriptType${key}-${value}`" :value="key">
                                    {{
                                        value }}</option>
                            </select>
                        </div>
                ...
                <transition name="fade-left">
                    <div v-if="currentStep > 1" class="column is-narrow">
                        <div class="box">
                            <p>Awesome! Now that you have the script created, let's run it. Click the "Run Script"
                                button.
                            </p>
                            <button @click="onScriptRun" :disabled="isScriptRunning"
                                :class="{ 'is-loading': isScriptRunning }" class="button is-primary">Run Script</button>
                            <div class="step">
                ...
                <transition name="fade-left">
                    <div v-if="currentStep > 2" class="column is-narrow">
                        <div class="box">
                            <p>Awesome! The script is now running in AWS Lambda. Check out the script output in "Script
                                Results"
                                tab.</p>
                            <div class="step">
...
import { BindProp, BindSelectedCopy } from '@/decorator/bindable';
...
@Component({
    components: { SuccessTick, IsFreeTier }
})
