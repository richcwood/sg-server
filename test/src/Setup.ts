...
export default class TestSetup {
    ...
    public async CreatePaymentMethod(team: TeamSchema) {
        ...
        await this.RestAPICall('paymentMethod', 'POST', team.id.toHexString(), null, {
            _teamId: team.id,
    ...
    async CreateAgents() {
        ...
        agentProperties = {
            _teamId: team.id,
            machineId: 'TestAgent3',
            ipAddress: '10.10.0.92',
            tags: { throwingstar: 'true' },
            numActiveTasks: 0,
            lastHeartbeatTime: null,
            rmqPassword: team['rmqPassword'],
        };
        ...
        agentProperties = {
            _teamId: team.id,
            machineId: 'TestAgent4',
            ipAddress: '10.10.0.93',
            tags: { numchucks: 'true', throwingstar: 'true' },
            numActiveTasks: 1,
            lastHeartbeatTime: null,
            rmqPassword: team['rmqPassword'],
        };
    ...
    async InitAgent(agent: any) {
        try {
            ...
            if ('inactiveAgentJob' in agent) params.inactiveAgentJob = agent.inactiveAgentJob;
    ...
    async CleanupTest(test: any) {
        try {
            ...
            for (let i = 0; i < test.jobs.length; i++) {
                let tasks: any = await this.mongoRepo.GetManyByQuery(
                    { _jobId: this.mongoRepo.ObjectIdFromString(test.jobs[i]['id']) },
                    'task'
                );
                for (let j = 0; j < tasks.length; j++) {
                    if (tasks[j].artifacts) {
                        for (let k = 0; k < tasks[j].artifacts.length; k++) {
                            let artifactToDelete: any = await this.RestAPICall(
                                `artifact/${tasks[j].artifacts[k]}`,
                                'GET',
                                tasks[j]._teamId,
                                null,
                                null
    ...
    protected async UploadFileToS3(
        ...
        return new Promise(async (resolve, reject) => {
            try {
                let res: any = await this.RestAPICall('artifact', 'POST', _teamId, null, {
                    name: path.basename(compressedFilePath),
                    type: fileType,
                });
