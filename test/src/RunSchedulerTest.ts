...
process.on('unhandledRejection', (reason, p) => {
    logger.LogError('RunSchedulerTest Unhandled Rejection', { 'Promise': util.inspect(p, false, null), 'Reason': util.inspect(reason, false, null) });
});
...
(async () => {
    try {
        ...
        fs.readdir(testsRootPath, async (err, files) => {
            ...
            for (let f of files) {
                try {
                    if (f.startsWith('Test') && (!(f.endsWith('.js.map'))) && (f != 'TestBase.js') && (f != 'TestArtifacts.js') && ((testsToRun.indexOf('ALL') >= 0) || (testsToRun.indexOf(f) >= 0))) {
                        ...
                        if (!res) {
                            await logger.LogError('Test failed', { 'Description': testModule.description, 'Location': testsRootPath + '/' + f });
                        } else {
                            await logger.LogError('Test passed', { 'Description': testModule.description });
                        }
