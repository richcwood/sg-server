...
export default class Test49 extends TestBase.WorkflowTestBase {
    ...
    public async RunTest() {
        ...
        resApiCall = await this.testSetup.RestAPICall(
            `taskoutcomeaction/interrupt/${taskOutcome[0].model.id}`,
            'POST',
            _teamId,
            null
        );
        if (resApiCall.data.statusCode != 200) {
            self.logger.LogError('Failed', {
                Message: `taskoutcomeaction/interrupt/${taskOutcome[0].model.id} POST returned ${resApiCall.data.statusCode}`,
            });
            return false;
        }
