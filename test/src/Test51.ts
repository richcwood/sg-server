...
export default class Test51 extends TestBase.WorkflowTestBase {
    ...
    constructor(testSetup) {
        ...
        this.description = 'Auto restart tasks test - client disconnected, run orphaned tasks script, should restart on another agent';
    ...
    public ProcessOrphanedTasks = async () => {
        try {
            ...
            const activeAgentTimeoutSeconds = 10;
    ...
    public async RunTest() {
        ...
        const stepOutcome = _.filter(self.bpMessages, x => x.domainType == 'StepOutcome' && x.operation == 1 && x.model._taskOutcomeId == taskOutcome[0].model.id);
        ...
        const newTaskOutcome = _.filter(self.bpMessages, x => x.domainType == 'TaskOutcome' && x.operation == 1 && x.model._taskId == task[0].model.id && x.model.id != taskOutcome[0].model.id);
