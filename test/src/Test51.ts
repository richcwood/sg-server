...
export default class Test51 extends TestBase.WorkflowTestBase {
    ...
    public async RunTest() {
        ...
        const stepOutcome = _.filter(
            self.bpMessages,
            (x) =>
                x.domainType == 'StepOutcome' && x.operation == 1 && x.model._taskOutcomeId == taskOutcome[0].model.id
        );
        ...
        let agents = _.filter(
            self.testSetup.agents,
            (a) => a.instanceId.toHexString() == taskOutcome[0].model._agentId
        );
        await agents[0].Stop();
