...
export default class Test66 extends TestBase.WorkflowTestBase {
    ...
    public async RunTest() {
        ...
        if (resApiCall.data.statusCode != 201) {
            self.logger.LogError('Failed', { Message: `job POST returned ${resApiCall.data.statusCode}`, _jobDefId: jobDefs['Job 66'].id });
            return false;
        }
        ...
        resApiCall = await this.testSetup.RestAPICall(`jobdef/${jobDefs['Job 66'].id}`, 'PUT', _teamId, null, { status: JobDefStatus.RUNNING });
        ...
        const task1 = _.filter(self.bpMessages, x => x.domainType == 'Task' && x.operation == 1 && x.model._jobId == job.id && x.model.name == properties.jobDefs[0].taskDefs[0].name);
        ...
        const task1Outcome = _.filter(self.bpMessages, x => x.domainType == 'TaskOutcome' && x.operation == 1 && x.model._taskId == task1[0].model.id);
