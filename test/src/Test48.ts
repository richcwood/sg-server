...
export default class Test48 extends TestBase.WorkflowTestBase {
    ...
    public async RunTest() {
        ...
        const taskOutcome = _.filter(
            self.bpMessages,
            (x) => x.domainType == 'TaskOutcome' && x.operation == 1 && x.model._jobId == firstJob.job.id
        );
        ...
        resApiCall = await this.testSetup.RestAPICall(
            `taskoutcomeaction/restart/${taskOutcome[0].model.id}`,
            'POST',
            _teamId,
            null
        );
        if (resApiCall.data.statusCode != 200) {
            self.logger.LogError('Failed', {
                Message: `taskoutcomeaction/restart/${taskOutcome[0].model.id} POST returned ${resApiCall.data.statusCode}`,
            });
            return false;
        }
        ...
        const taskOutcomeRestarted = _.filter(
            self.bpMessages,
            (x) =>
                x.domainType == 'TaskOutcome' &&
                x.operation == 1 &&
                x.model._jobId == firstJob.job.id &&
