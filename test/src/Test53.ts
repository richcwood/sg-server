...
export default class Test53 extends TestBase.WorkflowTestBase {
    ...
    constructor(testSetup) {
        ...
        this.description = 'Multi-task job - first task succeeds - interrupt job - only running task should be interrupted - restart job';
    ...
    public async RunTest() {
        ...
        let job;
        resApiCall = await this.testSetup.RestAPICall(`job`, 'POST', _teamId, { _jobDefId: jobDefs['Job 53'].id, correlationId: correlationId });
        if (resApiCall.data.statusCode != 201) {
            self.logger.LogError('Failed', { Message: `job POST returned ${resApiCall.data.statusCode}`, _jobDefId: jobDefs['Job 53'].id });
            return false;
        }
