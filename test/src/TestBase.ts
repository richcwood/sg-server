...
export default abstract class TestBase {
                ...
                try {
                    ...
                    if (testComplete) {
                        if ((Date.now() - testCompleteTime) > self.maxWaitTimeAfterJobComplete) {
                            console.log(`Max wait time after job complete exceeded - '${self.maxWaitTimeAfterJobComplete}'`);
                            resolve();
                            return;
                        }
        ...
        for (let task of self.taskDefs) {
            ...
            if ((ev.cntPartialMatch + ev.cntFullMatch) != ev.matchCount) {
                if (ev.matchType && ev.matchType == '>=') {
                    if ((ev.cntPartialMatch + ev.cntFullMatch) < ev.matchCount) {
                        taskPassed = false;
                        self.logger.LogError('Task failed', { 'Error': `Received ${ev.cntPartialMatch + ev.cntFullMatch} messages, at least ${ev.matchCount} expected`, 'Description': self.description, 'Task': JSON.stringify(task, null, 4) });
                    }
                } else {
                    taskPassed = false;
                    self.logger.LogError('Task failed', { 'Error': `Received ${ev.cntPartialMatch + ev.cntFullMatch} messages, ${ev.matchCount} expected`, 'Description': self.description, 'Task': JSON.stringify(task, null, 4) });
                }
            }
            if ((ev.type == 'task') && !ev.tagsMatch) {
                taskPassed = false;
                self.logger.LogError('Task failed', { 'Error': `Task executed by Agent without required tags`, 'Description': self.description, 'Task': util.inspect(task, false, null) });
            }
            if (taskPassed) {
    ...
    protected async OnBrowserPush(params: any, msgKey: string, ch: any) {
        ...
        if (params.domainType == 'Job') {
            ...
            if (params.operation == PayloadOperation.CREATE) {
                ...
                if (!this.taskOutcomes[params.model.id])
                    return;
        ...
        if ('values' in ev) {
            for (let i = 0; i < Object.keys(ev.values).length; i++) {
                ...
                if (!(keyVal in taskOutcome)) {
                    self.logger.LogError('Task failed', { 'Task': taskOutcome.name, 'Error': 'Missing task value', 'Description': self.description, 'Key': keyVal, 'Expected': ev.values[keyVal] });
                } else if (ev.values[keyVal] != taskOutcome[keyVal]) {
                    self.logger.LogError('Task failed', { 'Task': taskOutcome.name, 'Error': 'Unexpected task value', 'Description': self.description, 'Key': keyVal, 'Expected': ev.values[keyVal], 'Actual': taskOutcome[keyVal] });
                } else {
                    cntMatchedValues += 1;
                }
            };
        ...
        if ('step' in ev) {
            ...
            for (let i = 0; i < ev.step.length; i++) {
                ...
                if (!step_matches || step_matches.length < 1) {
                    self.logger.LogError('Task failed', { 'Task': taskOutcome.name, 'Error': `Step '${ev_step.name}' not in task step outcomes`, 'Description': self.description });
                } else {
                    ...
                    for (let i = 0; i < Object.keys(ev_step.values).length; i++) {
                        ...
                        if (ev_step.values[keyVal] != step[keyVal]) {
                            self.logger.LogError('Task failed', { 'Task': taskOutcome.name, 'Error': 'Unexpected task step value', 'Description': self.description, 'Key': ev_step.name + '.' + keyVal, 'Expected': ev_step.values[keyVal], 'Actual': step[keyVal] });
                        } else {
                            cntMatchedValues += 1;
                        }
                    };
    ...
    protected async CompletedTaskHandler(taskOutcome: TaskOutcomeSchema) {
        ...
        if (task.name == 'InactiveTask') {
            ...
            if (!taskDef || taskDef.length < 1) {
                self.logger.LogError('Task failed', { 'Task': task.name, 'Error': `Task definition 'InactiveAgentTask' not in test taskDef array`, 'Description': self.description });
            } else {
            ...
            const taskDef: TaskDefSchema = _.filter(self.taskDefs, x => x._jobDefId == jobDef.id && x.name == task.name)[0];
            ...
            if (taskDef.requiredTags) {
                ...
                for (let i = 0; i < Object.keys(taskDef.requiredTags).length; i++) {
                    ...
                    if (!(key in executingAgent['tags']) || (taskDef.requiredTags[key] != executingAgent['tags'][key])) {
                        ev.tagsMatch = false;
                        self.logger.LogError('Task failed', { 'Task': task.name, 'Error': `Executing agent '${taskOutcome._agentId}" missing required tag "${key}'`, 'Description': self.description });
                    }
                }
            }
        }
    ...
    protected async CompletedJobsHandler(jobOutcome: JobSchema) {
        if (jobOutcome.status < Enums.JobStatus.COMPLETED)
            return;
        ...
        Object.keys(ev.values).forEach((keyVal) => {
            if (!(keyVal in jobOutcome)) {
                self.logger.LogError('Job failed', { 'job': job.name, 'Error': 'Missing job value', 'Description': self.description, 'Key': keyVal, 'Expected': ev.values[keyVal] });
            } else if (ev.values[keyVal] != jobOutcome[keyVal]) {
                self.logger.LogError('Job failed', { 'job': job.name, 'Error': 'Unexpected job value', 'Description': self.description, 'Key': keyVal, 'Expected': ev.values[keyVal], 'Actual': jobOutcome[keyVal] });
            } else {
                cntMatchedValues += 1;
            }
        });

        if (cntMatchedValues < Object.keys(ev.values).length)
            ev.cntPartialMatch += 1;
        else if (cntMatchedValues == Object.keys(ev.values).length)
            ev.cntFullMatch += 1;
    }
        ...
        for (let i = 0; i < adhocTaskTestBaseInstance.tasks.length; i++) {
            ...
            if ((ev.cntPartialMatch + ev.cntFullMatch) != ev.matchCount) {
                if (ev.matchType && ev.matchType == '>=') {
                    if ((ev.cntPartialMatch + ev.cntFullMatch) < ev.matchCount) {
                        taskPassed = false;
                        adhocTaskTestBaseInstance.logger.LogError('Task failed', { 'Error': `Received ${ev.cntPartialMatch + ev.cntFullMatch} messages, at least ${ev.matchCount} expected`, 'Description': adhocTaskTestBaseInstance.description, 'Task': adhocTaskTestBaseInstance.tasks[i] });
                    }
                } else {
                    taskPassed = false;
                    adhocTaskTestBaseInstance.logger.LogError('Task failed', { 'Error': `Received ${ev.cntPartialMatch + ev.cntFullMatch} messages, ${ev.matchCount} expected`, 'Description': adhocTaskTestBaseInstance.description, 'Task': adhocTaskTestBaseInstance.tasks[i] });
                }
            }
            if ((ev.type == 'task') && !ev.tagsMatch) {
                taskPassed = false;
                adhocTaskTestBaseInstance.logger.LogError('Task failed', { 'Error': `Task executed by Agent without required tags`, 'Description': adhocTaskTestBaseInstance.description, 'Task': adhocTaskTestBaseInstance.tasks[i] });
            }
            if (taskPassed) {
                adhocTaskTestBaseInstance.logger.LogDebug('Task passed', { 'Description': adhocTaskTestBaseInstance.description, 'Task': adhocTaskTestBaseInstance.tasks[i] });
            }
...
export abstract class WorkflowTestBase extends TestBase {
    ...
    public async StartTestMonitor() {
        ...
        await wftInst.amqp.ConsumeQueue(wftInst.rmqDLQRoute, false, true, false, true, wftInst.OnDLQMessage.bind(this), '');
    }
    ...
    protected async WaitForTestToComplete(maxWaitTime: number = 30000) {
        let testStartTime = Date.now();
        ...
        await new Promise<void>(async (resolve, reject) => {
            while (true) {
                try {
                    ...
                    let unmatchedBp: any[] = _.filter(wftInst.bpMessagesExpected, x => !x.success);
                    let unmatchedDlq: any[] = _.filter(wftInst.dlqMessagesExpected, x => !x.success);
                    testComplete = ((unmatchedBp.length + unmatchedDlq.length) == 0);
