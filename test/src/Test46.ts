    ...
    public async RunTest() {
        ...
        if (resApiCall.data.statusCode != 400) {
            self.logger.LogError('Failed', {
                Message: `jobaction/restart/${firstJob.job.id} POST returned ${resApiCall.data.statusCode} - should have returned 400`,
            });
            return false;
        }
        ...
        if (resApiCall.data.errors[0].description != expectedError) {
            self.logger.LogError('Failed', {
                Message: `jobaction/restart/${firstJob.job.id} POST error description = "${resApiCall.data.errors[0].description}" - should have returned "${expectedError}"`,
            });
            return false;
        }
