...
export default class Test29 extends TestBase.default {
    ...
    public async CreateTest() {
        ...
        // /// Create org
        // let org: any = {'name': 'TestOrg29', 'isActive': true, 'rmqPassword': KikiUtils.makeid(10)};
        ...
        /// Create scripts
        let script_obj: ScriptSchema = {'_orgId': _orgId, 'name': 'Script 29', 'scriptType': Enums.ScriptType.PYTHON, 'code': script_b64, _originalAuthorUserId: this.sgUser.id, _lastEditedUserId: this.sgUser.id, lastEditedDate: new Date(), shadowCopyCode: script_b64 };
        script_obj = await self.CreateScript(script_obj, _orgId);
        self.scripts.push(script_obj);    
        let step2: StepDefSchema = {'_orgId': _orgId, '_taskDefId': taskDef.id, 'name': 'step2', '_scriptId': script_obj['id'], 'order': 0, 'arguments': '@kpg("globalParam1")'};
