              ...
              if (arrParams) {
                for (let i = 0; i < arrParams.length; i++) {
                  try {
                    ...
                    console.log(
                      `Error parsing stdout @sgo capture for string \"${arrParams[i].substring(4)}\": ` + e.message,
                      e.stack,
                      {}
                    );
                  }
...
let AgentRestAPICall = async () => {
  for (let i = 0; i < 50; i++) {
    const response: any = await axios({
      ...
      headers: {
        Cookie:
          "Auth=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJvcmdJZHMiOlsiNWRlOTVjMDQ1MzE2MmU4ODkxZjVhODMwIl0sImVtYWlsIjoiQWdlbnQtVGVzdEFnZW50MyIsImV4cCI6MTU4Nzc0NTI2OSwiaWF0IjoxNTg3NjU4ODY5fQ.cGOxmXLSoK6D5lV3G3wynuGEs4w-f2PVwfzEFd4map0;",
        _teamId: "5de95c0453162e8891f5a830",
      },
    });
