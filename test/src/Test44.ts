...
export default class Test44 extends TestBase.WorkflowTestBase {
    ...
    public async RunTest() {
        ...
        if (resApiCall.data.statusCode != 400) {
            self.logger.LogError('Failed', {
                Message: `jobaction/restart/${firstJob.job.id} POST returned ${resApiCall.data.statusCode}`,
            });
            return false;
        } else if (
            resApiCall.data.errors[0].description !=
            `Job ${firstJob.job.id} cannot be restarted since current status is "COMPLETED"`
        ) {
            self.logger.LogError('Failed', {
                Message: `jobaction/restart/${firstJob.job.id} POST - incorrect description "${resApiCall.data.error[0].description}"`,
            });
            return false;
        }
