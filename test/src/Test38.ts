...
export default class Test38 extends TestBase.WorkflowTestBase {
    ...
    public async RunTest() {
        ...
        resApiCall = await self.testSetup.RestAPICall(
            `jobaction/cancel/${secondJob.job.id}`,
            'POST',
            _teamId,
            null,
            null
        ...
        if (resApiCall.data.statusCode != 200) {
            self.logger.LogError('Failed', {
                Message: `jobaction/cancel/${secondJob.job.id} POST returned ${resApiCall.data.statusCode}`,
                secondJob,
            });
            return false;
        }
        ...
        resApiCall = await self.testSetup.RestAPICall(
            `jobaction/cancel/${firstJob.job.id}`,
            'POST',
            _teamId,
            null,
            null
        ...
        if (resApiCall.data.statusCode != 200) {
            self.logger.LogError('Failed', {
                Message: `jobaction/cancel/${firstJob.job.id} POST returned ${resApiCall.data.statusCode}`,
                firstJob,
            });
            return false;
        }
