...
export default class Test67 extends TestBase.WorkflowTestBase {
    ...
    public async RunTest() {
        ...
        if (resApiCall.data.statusCode != 201) {
            self.logger.LogError('Failed', { Message: `job POST returned ${resApiCall.data.statusCode}`, _jobDefId: jobDefs['Job 67'].id });
            return false;
        }
        ...
        resApiCall = await this.testSetup.RestAPICall(`jobdef/${jobDefs['Job 67'].id}`, 'PUT', _teamId, null, { status: JobDefStatus.RUNNING });
