...
if (fileName.indexOf('Test') != 0) path = fileName;

let numMessagesToPushOnStart = 0;
if (process.argv.length > 3) numMessagesToPushOnStart = parseInt(process.argv[3]);
...
let SendNextMessage = async (row: number) => {
    rl.question(
        `Next message (${row + 1}/${numMessagesTotal}) - domain: ${bpMessages[row].domainType}, operation: ${
            PayloadOperation[bpMessages[row].operation]
        }\n\n`,
        async (name) => {
            const msg = bpMessages[row];
            await rabbitMQPublisher.publish(_teamId, msg.domainType, msg.correlationId, msg.operation, msg.model);
            row += 1;
            if (row >= numMessagesTotal) rl.close();
            else SendNextMessage(row);
        }
    );
};
...
(async () => {
    for (row = 0; row < numMessagesToPushOnStart; row++) {
        if (row >= numMessagesTotal) process.exit();
        const msg = bpMessages[row];
        console.log(
            `Pushing message (${row + 1}/${numMessagesTotal}) - domain: ${msg.domainType}, operation: ${
                PayloadOperation[msg.operation]
            }`
        );
        await rabbitMQPublisher.publish(_teamId, msg.domainType, msg.correlationId, msg.operation, msg.model);
    }
