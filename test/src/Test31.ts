...
export default class Test31 extends TestBase.default {
    ...
    public async CreateTest() {
        ...
        // /// Create org
        // let org: any = {'name': 'TestOrg31', 'isActive': true, 'rmqPassword': KikiUtils.makeid(10)};
        ...
        /// Create scripts
        let script_obj1: ScriptSchema = { '_orgId': _orgId, 'name': 'Script 31.1', 'scriptType': Enums.ScriptType.PYTHON, 'code': script1_b64, _originalAuthorUserId: this.sgUser.id, _lastEditedUserId: this.sgUser.id, lastEditedDate: new Date(), shadowCopyCode: script1_b64 };
        ...
        let script_obj2: ScriptSchema = {'_orgId': _orgId, 'name': 'Script 31.2', 'scriptType': Enums.ScriptType.PYTHON, 'code': script2_b64, _originalAuthorUserId: this.sgUser.id, _lastEditedUserId: this.sgUser.id, lastEditedDate: new Date(), shadowCopyCode: script2_b64 };
        script_obj2 = await self.CreateScript(script_obj2, _orgId);
        self.scripts.push(script_obj2);    
        let step2: StepDefSchema = {'_orgId': _orgId, '_taskDefId': taskDef1.id, 'name': 'step2', '_scriptId': script_obj2['id'], 'order': 1, 'arguments': '@kpg("globalParam1")'};
