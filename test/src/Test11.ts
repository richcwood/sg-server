...
export default class Test11 extends TestBase.default {
    ...
    public async CreateTest() {
        ...
        // /// Create team
        // let team: any = {'name': 'TestTeam11', 'isActive': true, 'rmqPassword': SGUtils.makeid(10)};
        ...
        let taskDef1: TaskDefSchema = {'_teamId': _teamId, 'name': 'Task1_1', '_jobDefId': jobDef.id, 'target': Enums.TaskDefTarget.SINGLE_AGENT, 'requiredTags': {}, 'fromRoutes': []};
        taskDef1 = await self.CreateTaskDef(taskDef1, _teamId);

        let taskDef3: TaskDefSchema = {'_teamId': _teamId, 'name': 'Task1_3', '_jobDefId': jobDef.id, 'target': Enums.TaskDefTarget.ALL_AGENTS, 'requiredTags': {}, 'fromRoutes': [['Task1_1','ok']]};
        taskDef3 = await self.CreateTaskDef(taskDef3, _teamId);

        /// Create scripts
        let script_obj1: ScriptSchema = { '_teamId': _teamId, 'name': 'Script 11.1', 'scriptType': Enums.ScriptType.PYTHON, 'code': script1_b64, _originalAuthorUserId: this.sgUser.id, _lastEditedUserId: this.sgUser.id, lastEditedDate: new Date(), shadowCopyCode: script1_b64 };
        ...
        let script_obj3: ScriptSchema = {'_teamId': _teamId, 'name': 'Script 11.3', 'scriptType': Enums.ScriptType.PYTHON, 'code': script3_b64, _originalAuthorUserId: this.sgUser.id, _lastEditedUserId: this.sgUser.id, lastEditedDate: new Date(), shadowCopyCode: script1_b64 };
        script_obj3 = await self.CreateScript(script_obj3, _teamId);
        self.scripts.push(script_obj3);    
        let step3: StepDefSchema = {'_teamId': _teamId, '_taskDefId': taskDef3.id, 'name': 'step1', '_scriptId': script_obj3['id'], 'order': 0, 'arguments': ''};
