...
export default class Test64 extends TestBase.WorkflowTestBase {
    constructor(testSetup) {
        ...
        this.description =
            'Run multiple jobs with task targeting "all agents with tags" - should round robin across all qualified agents';
    ...
    public async RunTest() {
        ...
        for (let i = 0; i < 2; ++i) {
            ...
            resApiCall = await this.testSetup.RestAPICall(
                `taskoutcome?filter=_jobId==${job.job.id}&responseFields=id&limit=1`,
                'GET',
                _teamId,
                null
            );
            if (resApiCall.data.statusCode != 200) {
                self.logger.LogError('Failed', {
                    Message: `taskoutcome?filter=_jobId==${job.job.id}&responseFields=id&limit=1 GET returned ${resApiCall.data.statusCode}`,
                });
                return false;
            }
        ...
        for (let i = 0; i < 2; ++i) {
            ...
            const taskOutcome = _.filter(
                self.bpMessages,
                (x) => x.domainType == 'TaskOutcome' && x.operation == 1 && x.model.id == taskOutcomeId
            );
