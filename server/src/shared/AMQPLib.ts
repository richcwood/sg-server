...
export class AMQPConnector {
    ...
    LogError(msg: string, stackTrace: string, values: any) {
        this.logger.LogError(
            msg,
            Object.assign({ _appName: this.appName, Stack: stackTrace, ClientId: this.clientId }, values)
        );
    }
    ...
    async CreatePublisherChannel(): Promise<boolean> {
        ...
        this.pubChannel.on('return', (val) => {
            if (
                ...
                (val.fields.routingKey == 'bp' || val.fields.routingKey == 'ba') &&
                val.fields.replyCode &&
                val.fields.replyCode == 312
            )
                return;
            this.LogDebug('Publisher channel message returned', {
                fields: val.fields,
                properties: val.properties,
                content: val.content.toString('utf8'),
            });
        });
    ...
    async AssertExchange(exchange: string, type: string, durable: boolean, internal: boolean, autoDelete: boolean) {
        if (!exchange || exchange == '') return;

        let options: any = { durable: durable, internal: internal, autoDelete: autoDelete };
        let res;
        if (this.pubChannel) res = await this.pubChannel.assertExchange(exchange, type, options);
        else if (this.consumerChannel) res = await this.consumerChannel.assertExchange(exchange, type, options);
    }
    ...
    async ConsumeQueue(
        ...
        try {
            ...
            sub = await this.consumerChannel.consume(
                queueName,
                (msg) => {
                    if (msg != null) {
                        try {
                            this.LogDebug('Message received', {
                                Fields: util.inspect(msg.fields),
                                Properties: util.inspect(msg.properties),
                                ...
                                (ok, msgKey) => {
                                    if (!noAck) {
                                        if (this.activeMessages.indexOf(msgKey) > -1) {
                                            try {
                                                if (ok) this.consumerChannel.ack(msg);
                                                else this.consumerChannel.reject(msg, true);
                                            } catch (e) {
                                                this.LogError('Error occurred acking AMQP message', '', {
                                                    error: e,
            ...
            this.subscribedRoutes.push({
                queueName: queueName,
                exclusive: exclusive,
                durable: durable,
                autoDelete: autoDelete,
                noAck: noAck,
                fnHandleMessage: fnHandleMessage,
                exchange: exchange,
                expires: expires,
            });
    ...
    async ConsumeRoute(
        ...
        try {
            ...
            this.LogError(`Error consuming AMQP route: ${e.message}`, e.stack, {
                QueueName: queueName,
                Exchange: exchange,
                Route: route,
            });
