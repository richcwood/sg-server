...
export class StompConnector {
    ...
    async OnDisconnect() {
        ...
        this.lock.acquire('restart', async (done) => {
            try {
                if (!this.stoppedByUser && this.connectedToStomp) return;
    ...
    async Publish(exchange: string, routingKey: string, content: any, args: any = {}) {
        try {
            let route: string;
            if (!exchange || exchange == '') route = `/queue/${routingKey}`;
            else route = `/exchange/${exchange}/${routingKey}`;
            this.stompClient.send(route, args, JSON.stringify(content));
            this.LogDebug('Published message', {
                Content: util.inspect(content, false, null),
                Exchange: exchange,
                Route: route,
            });
        } catch (e) {
            if (this.stoppedByUser) throw e;
