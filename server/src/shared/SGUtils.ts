...
export class SGUtils {
                ...
                try {
                    ...
                    if (!runtimeVars[varKey]) {
                        if (varKey in job.runtimeVars) {
                            ...
                            const teamVar = await teamVariableService.findTeamVariableByName(
                                _teamId,
                                varKey,
                                'value sensitive'
                            );
                    ...
                    throw new Error(
                        `Error in script @sgg capture for string \"${arrFindVarsScript[i]}\": ${e.message}`
                    );
                }
            }
        }
        ...
        if (arrFindScriptsToInject) {
            for (let i = 0; i < arrFindScriptsToInject.length; i++) {
                try {
                    ...
                    throw new Error(
                        `Error in script @sgs capture for string \"${arrFindScriptsToInject[i]}\": ${e.message}`
                    );
                }
            }
        }
        ...
        for (let i = 0; i < taskDefs.length; i++) {
            ...
            if (taskDef.fromRoutes) {
                for (let i = 0; i < taskDef.fromRoutes.length; i++) {
                    ...
                    if (up_task in graph)
                        if (graph[up_task].indexOf(taskDef.name) < 0) graph[up_task].push(taskDef.name);
                }
            }
            if (taskDef.toRoutes) {
                ...
                try {
                    if (err) {
                        ...
                        sgMail.send({
                            ...
                            subject: `SaaSGlue invoice for period ending ${moment(invoiceModel.endDate).format(
                                'MMMM d, YYYY'
                            )}`,
                            attachments: [
                                {
    ...
    static SendTeamSharedInviteEmail = async (
        ...
        let contentHtml: string = SGUtils.GenerateTeamSharedInviteContent(
            'html',
            team,
            acceptInviteLink,
            recipientAddress
        ...
        let contentText: string = SGUtils.GenerateTeamSharedInviteContent(
            'text',
            team,
            acceptInviteLink,
            recipientAddress
    ...
    static OnJobComplete = async (_teamId: mongodb.ObjectId, job: any, logger: BaseLogger) => {
        ...
        if (jobCompleteAlertSlackURL)
            SGUtils.SendJobCompleteAlertSlack(
                Enums.JobStatus[job.status],
                url,
                jobName,
                jobCompleteAlertSlackURL,
                logger
            );
    };
