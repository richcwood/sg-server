...
export class SGUtils {
  ...
  static async GenerateDownstreamDependenciesForJobTasks(tasks: TaskDefSchema[]) {
    ...
    for (let i = 0; i < tasks.length; i++) {
      ...
      if (Object.keys(downstreamDependencies).indexOf(task.name) < 0) downstreamDependencies[task.name] = [];
    }
    ...
    for (let i = 0; i < tasks.length; i++) {
      ...
      if (task[SGStrings.fromRoutes]) {
        for (let i = 0; i < task[SGStrings.fromRoutes].length; i++) {
          ...
          let routePattern = "";
          if (taskRoute[1]) routePattern = taskRoute[1];
        ...
        try {
          ...
          if (!runtimeVars[varKey]) {
            if (varKey in job.runtimeVars) {
              ...
              const teamVar = await teamVariableService.findTeamVariableByName(_teamId, varKey, "value sensitive");
    ...
    for (let i = 0; i < taskDefs.length; i++) {
      ...
      if (taskDef.fromRoutes) {
        for (let i = 0; i < taskDef.fromRoutes.length; i++) {
          ...
          if (up_task in graph) if (graph[up_task].indexOf(taskDef.name) < 0) graph[up_task].push(taskDef.name);
        }
      }
      if (taskDef.toRoutes) {
  ...
  static validateJob(job: any) {
    ...
    if (Object.keys(cd).length > 0)
      throw new ValidationError(
        `job contains a cyclic dependency with the following tasks: ${Object.keys(cd).filter((key) => cd[key])}`
      );
    ...
    for (let i = 0; i < job.tasks.length; i++) {
      ...
      if (task.target == Enums.TaskDefTarget.SINGLE_SPECIFIC_AGENT) {
        if (!task.targetAgentId)
          throw new ValidationError(
            `Task "${task.name}" target is "SINGLE_SPECIFIC_AGENT" but targetAgentId is missing`
          );
      } else if (
        task.target &
        ...
        if (!_.isPlainObject(task.requiredTags) || Object.keys(task.requiredTags).length === 0) {
          if (_.isPlainObject(task.requiredTags))
            throw new ValidationError(
              `Task "${task.name}" target is "SINGLE_AGENT_WITH_TAGS" or "ALL_AGENTS_WITH_TAGS" but no required tags are specified`
            );
          else
            throw new ValidationError(
              `Task "${task.name}" target is "SINGLE_AGENT_WITH_TAGS" or "ALL_AGENTS_WITH_TAGS" but required tags are incorrectly formatted`
            );
        }
      }
      ...
      if (
        task.autoRestart &&
        task.target & (Enums.TaskDefTarget.ALL_AGENTS | Enums.TaskDefTarget.ALL_AGENTS_WITH_TAGS)
      ) {
        throw new ValidationError(
          `Task "${task.name}" has autoRestart=true but target is "ALL_AGENTS" or "ALL_AGENTS_WITH_TAGS" - autoRestart tasks must target any single agent, a specific agent or a single agent with required tags`
        );
      }
    }
  }
  ...
  static GenerateInvoice = async (
    ...
    let team: TeamSchema = await teamService.findTeam(
      _teamId,
      "name billing_address1 billing_address2 billing_city billing_state billing_zip billing_email"
    );
    let invoice_raw: string;
    if (format == "html") invoice_raw = fs.readFileSync("server/src/resources/invoice_template.html", "utf8");
    else invoice_raw = fs.readFileSync("server/src/resources/invoice_template.txt", "utf8");

    let customer_info = team.name;
    if (team.billing_address1) customer_info += `<br>${team.billing_address1}`;
    if (team.billing_address2) customer_info += `<br>${team.billing_address2}`;
    if (team.billing_city) customer_info += `<br>${team.billing_city}`;
    if (team.billing_state) customer_info += `<br>${team.billing_state}`;
    if (team.billing_zip) customer_info += `<br>${team.billing_zip}`;
    if (team.billing_email) customer_info += `<br>${team.billing_email}`;
    ...
    if (scriptTotal < 0.01) scriptTotal = 0;
    let scriptRate = "0";
    if (invoice.scriptRate > 0) scriptRate = invoice.scriptRate.toFixed(6);
    ...
    if (storageTotal < 0.01) storageTotal = 0;
    let storageMB = "0";
    if (invoice.storageMB > 0) storageMB = invoice.storageMB.toFixed(8);
    ...
    if (invoice.artifactsDownloadedGB > 0) artifactsDownloadedGB = invoice.artifactsDownloadedGB.toFixed(8);
    ...
    if (artifactsStorageTotal < 0.01) artifactsStorageTotal = 0;
    let artifactsStorageGB = "0";
    if (invoice.artifactsStorageGB > 0) artifactsStorageGB = invoice.artifactsStorageGB.toFixed(8);
    ...
    if (awsLambdaRequestsTotal < 0.01) awsLambdaRequestsTotal = 0;
    ...
    if (awsLambdaComputeGBSecondsTotal < 0.01) awsLambdaComputeGBSecondsTotal = 0;
    let awsLambdaComputeGBSeconds = "0";
    if (invoice.awsLambdaComputeGbSeconds > 0) awsLambdaComputeGBSeconds = invoice.awsLambdaComputeGbSeconds.toFixed(8);
    invoice_raw = invoice_raw.replace("{aws_lambda_compute_gb_seconds}", "" + awsLambdaComputeGBSeconds);
    invoice_raw = invoice_raw.replace(
      "{aws_lambda_compute_gb_seconds_rate}",
      "" + invoice.awsLambdaComputeGbSecondsRate
    );
    invoice_raw = invoice_raw.replace(
      "{aws_lambda_compute_gb_seconds_total}",
      "" + awsLambdaComputeGBSecondsTotal.toFixed(2)
    );
  ...
  static CreateAndSendInvoice = async (
    team: TeamSchema,
    invoiceModel: any,
    paymentTransaction: any,
    logger: BaseLogger
  ) => {
    const invoiceDateString = `${moment(invoiceModel.endDate).format("MMMM")}_${moment(invoiceModel.endDate).format(
      "YYYY"
    )}`;
  ...
  static SendTaskInterruptedAlertSlack = async (
    task_name: string,
    job_url: string,
    job_name: string,
    url: string,
  ...
  static SendJobCompleteAlertSlack = async (
    job_status: string,
    job_url: string,
    job_name: string,
    url: string,
  ...
  static GenerateConfirmEmailAlreadyExistsContent = (format: string, userName: string, recipientAddress: string) => {
    let content: string;
    if (format == "html") content = fs.readFileSync("server/src/resources/confirm_email_already_exists.html", "utf8");
    else content = fs.readFileSync("server/src/resources/confirm_email_already_exists.txt", "utf8");
  ...
  static SendConfirmEmailAlreadyExistsEmail = async (
    userName: string,
  ...
  static GeneratePasswordResetInvalidEmailContent = (format: string, recipientAddress: string) => {
    let content: string;
    if (format == "html") content = fs.readFileSync("server/src/resources/password_reset_email_invalid.html", "utf8");
    else content = fs.readFileSync("server/src/resources/password_reset_email_invalid.txt", "utf8");
  ...
  static GeneratePasswordResetConfirmationContent = (format: string, recipientAddress: string) => {
    ...
    else content = fs.readFileSync("server/src/resources/password_reset_confirmation_email.txt", "utf8");
  ...
  static GeneratePasswordResetContent = (format: string, email: string, resetPasswordLink: string) => {
    let content: string;
    if (format == "html") content = fs.readFileSync("server/src/resources/password_reset_email_valid.html", "utf8");
    else content = fs.readFileSync("server/src/resources/password_reset_email_valid.txt", "utf8");
  ...
  static GenerateTeamSharedInviteContent = (
    format: string,
    team: TeamSchema,
    ...
    let content: string;
    if (format == "html") content = fs.readFileSync("server/src/resources/email_shared_invite_template.html", "utf8");
    else content = fs.readFileSync("server/src/resources/email_shared_invite_template.txt", "utf8");
  ...
  static SendTeamSharedInviteEmail = async (
    team: TeamSchema,
    recipientAddress: string,
    acceptInviteLink: string,
    logger: BaseLogger
  ...
  static GenerateTeamInviteContent = (
    format: string,
    team: TeamSchema,
    inviter: any,
    acceptInviteLink: string,
    ...
    let content: string;
    if (format == "html") content = fs.readFileSync("server/src/resources/email_invite_template.html", "utf8");
    else content = fs.readFileSync("server/src/resources/email_invite_template.txt", "utf8");
  ...
  static SendTeamInviteEmail = async (
    team: TeamSchema,
    inviter: any,
    recipientAddress: string,
    acceptInviteLink: string,
    ...
    let contentHtml: string = SGUtils.GenerateTeamInviteContent(
      "html",
      team,
      inviter,
      acceptInviteLink,
    ...
    let contentText: string = SGUtils.GenerateTeamInviteContent(
      "text",
      team,
      inviter,
      acceptInviteLink,
  ...
  static SendSignupConfirmEmail = async (confirmationCode: string, recipientAddress: string, logger: BaseLogger) => {
    ...
    let subject =
      "=?utf-8?Q?saasglue_confirmation_code=3a_" +
      confirmationCodeConverted.substring(0, 9) +
      "=2d" +
      confirmationCodeConverted.substring(9, 18) +
      "?=";
    ...
    contentHtml = contentHtml.replace(
      /{confirmation_code}/g,
      "" + confirmationCode.substring(0, 3) + "-" + confirmationCode.substring(3, 6)
    );
  ...
  static SendTaskErrorAlertEmail = async (
    task_name: string,
    error: string,
    exit_code,
    signal,
  ...
  static SendTaskInterruptedAlertEmail = async (
    task_name: string,
    job_url: string,
    job_name: string,
    recipientAddress: string,
  ...
  static SendJobCompleteAlertEmail = async (
    job_status: string,
    job_url: string,
    job_name: string,
    recipientAddress: string,
  ...
  static OnStepFailed = async (_teamId: mongodb.ObjectId, updatedStepOutcome: any, logger: BaseLogger) => {
    const taskOutcome: TaskOutcomeSchema = await taskOutcomeService.findTaskOutcome(
      _teamId,
      updatedStepOutcome._taskOutcomeId,
      "_id _jobId _taskId"
    );
    if (taskOutcome) {
      ...
      if (_.isArray(taskQuery) && taskQuery.length > 0) {
        ...
        const job: JobSchema = await jobService.findJob(
          _teamId,
          taskOutcome._jobId,
          "name runId onJobTaskFailAlertEmail onJobTaskFailAlertSlackURL"
        );
        if (job) {
          if (job.onJobTaskFailAlertEmail) taskFailAlertEmail = job.onJobTaskFailAlertEmail;
          if (job.onJobTaskFailAlertSlackURL) taskFailAlertSlackURL = job.onJobTaskFailAlertSlackURL;

          if (!taskFailAlertEmail || !taskFailAlertSlackURL) {
            const team: TeamSchema = await teamService.findTeam(
              _teamId,
              "onJobTaskFailAlertEmail onJobTaskFailAlertSlackURL"
            );
            if (!taskFailAlertEmail && team.onJobTaskFailAlertEmail) taskFailAlertEmail = team.onJobTaskFailAlertEmail;
          ...
          if (taskFailAlertEmail || taskFailAlertSlackURL) {
            ...
            if (job.runId) jobName += `- ${job.runId}`;
            ...
            if (taskFailAlertEmail)
              SGUtils.SendTaskErrorAlertEmail(
                taskName,
                updatedStepOutcome.stderr,
                updatedStepOutcome.exitCode,
                updatedStepOutcome.signal,
                url,
                jobName,
                taskFailAlertEmail,
                logger
            ...
            if (taskFailAlertSlackURL)
              SGUtils.SendTaskErrorAlertSlack(
                taskName,
                updatedStepOutcome.stderr,
                updatedStepOutcome.exitCode,
                updatedStepOutcome.signal,
  ...
  static OnTaskInterrupted = async (_teamId: mongodb.ObjectId, taskOutcome: any, job: any, logger: BaseLogger) => {
    let taskName: string = taskOutcome._id.toHexString();
    const taskQuery: any[] = await taskService.findTask(_teamId, taskOutcome._taskId, "name");
    if (_.isArray(taskQuery) && taskQuery.length > 0) taskName = taskQuery[0].name;
    ...
    if (job.onJobTaskInterruptedAlertEmail) jobTaskInterruptedAlertEmail = job.onJobTaskInterruptedAlertEmail;

    if (job.onJobTaskInterruptedAlertSlackURL) jobTaskInterruptedAlertSlackURL = job.onJobTaskInterruptedAlertSlackURL;

    if (!jobTaskInterruptedAlertEmail || !jobTaskInterruptedAlertSlackURL) {
      const team: TeamSchema = await teamService.findTeam(
        _teamId,
        "onJobTaskInterruptedAlertEmail onJobTaskInterruptedAlertSlackURL"
      );
  ...
  static OnJobComplete = async (_teamId: mongodb.ObjectId, job: any, logger: BaseLogger) => {
    ...
    if (job.onJobCompleteAlertEmail) jobCompleteAlertEmail = job.onJobCompleteAlertEmail;

    if (job.onJobCompleteAlertSlackURL) jobCompleteAlertSlackURL = job.onJobCompleteAlertSlackURL;

    if (!jobCompleteAlertEmail || !jobCompleteAlertSlackURL) {
      const team: TeamSchema = await teamService.findTeam(
        _teamId,
        "onJobCompleteAlertEmail onJobCompleteAlertSlackURL"
      );
      if (!jobCompleteAlertEmail && team.onJobCompleteAlertEmail) jobCompleteAlertEmail = team.onJobCompleteAlertEmail;
  ...
  static SendCustomerEmail = async (
    recipientAddress: string,
    subject: string,
    rawMessage: string,
    logger: BaseLogger
  ...
  static SendInternalEmail = async (
