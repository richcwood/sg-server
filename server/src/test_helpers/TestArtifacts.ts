...
let CreateJobDefsFromTemplates = async (
    ...
    for (let jobDefProperties of properties.jobDefs) {
        ...
        for (let taskDefProperties of jobDefProperties.taskDefs) {
            ...
            const taskDef = convertResponseData(
                TaskDefSchema,
                await CreateTaskDefFromTemplate(_teamId, taskDefProperties)
            );
            taskDef.stepDefs = {};
            ...
            for (let stepDefProperties of taskDefProperties.stepDefs) {
                ...
                const stepDef = convertResponseData(
                    StepDefSchema,
                    await CreateStepDefFromTemplate(_teamId, stepDefProperties)
                );
