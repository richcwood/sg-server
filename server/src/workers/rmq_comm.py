...
class AsyncConsumer(object):
    ...
    def connect(self):
        ...
        """
        self._logger.debug('Connecting to {0}'.format(self._url))
    ...
    def on_connection_closed(self, connection, reply_code, reply_text):
        ...
        if self._closing:
            ...
            self._logger.warning('Connection closed, reopening in 5 seconds: ({0}) {1}'.format(reply_code, reply_text))
    ...
    def on_channel_closed(self, channel, reply_code, reply_text):
        ...
        """
        self._logger.info('Channel {0} was closed: ({1}) {2}'.format(channel, reply_code, reply_text))
    ...
    def setup_exchange(self, exchange_name):
        ...
        """
        self._logger.info('Declaring {0} exchange {1}'.format(self._exchange_type, exchange_name))
    ...
    def setup_queue(self, queue_name):
        ...
        """
        self._logger.info('Declaring queue {0}'.format(queue_name))
    ...
    def on_queue_declareok(self, method_frame):
        ...
        if self._routing_key is not None:
            if self._exchange != '':
                self._logger.info('Binding {0} to {1} with {2}'.format(self._exchange, self._queue_name, self._routing_key))
    ...
    def on_consumer_cancelled(self, method_frame):
        ...
        """
        self._logger.warning('Consumer was cancelled remotely, shutting down: {0}'.format(method_frame))
        if self._channel:
            self._channel.close()
    ...
    def close_channel(self):
        ...
        """
        self._logger.info('Closing the RMQ channel')
        self._channel.close()
    ...
    def stop(self):
        ...
        """
        self._logger.debug('RMQ stopping')
        self._closing = True
        self.stop_consuming()
        ...
        self._logger.debug('RMQ stopped')

    def close_connection(self):
        """This method closes the connection to RabbitMQ."""
        self._logger.info('Closing RMQ connection')
        self._connection.close()
        if self._basic_publish_connection:
            self._basic_publish_connection.close()

    def create_basic_publish_channel(self, queue_name, routing_key):
    ...
    def basic_publish(self, msg, routing_key):
        ...
        self._logger.info('\n' + 'sent message to rabbitmq \"' + routing_key + '\" [' + msg_json + ']\n')
...
def main():
    ...
    num_threads = 1
    start_async_consumer_multithreaded('amqps://bartSpikeUser:RocketDog99!@funny-finch.rmq.cloudamqp.com/bartsVHost', cml_adapter, {
        'exch': 'mission',
        'exch_type': 'topic',
        'durable': True,
