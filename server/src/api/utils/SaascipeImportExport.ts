...
let PrepareTaskDefsForExport = async (
    ...
    for (let index = 0; index < taskDefsToExport.length; index++) {
        ...
        const getStepDefsResults: { stepDefs: Array<any>; scripts: Array<mongodb.Objectid> } =
...
let ImportScriptSaascipe = async (
    ...
    if (!saascipeVersion)
        throw new MissingObjectError(`SaascipeVersion ${_saascipeVersionId.toHexString()} not found.`);
...
let ImportSchedules = async (
    ...
    for (let schedule of schedulesToImport) {
        ...
        const newSchedule: ScheduleSchema = await scheduleService.createSchedule(
            _teamId,
            newScheduleData,
            correlationId
        );
    ...
    for (let taskDefToImport of taskDefsToImport) {
        ...
        if (taskDefToImport.target == TaskDefTarget.AWS_LAMBDA) {
            if (taskDefToImport.stepDefs.length < 1) {
                ...
                const stepDefs: StepDefSchema[] = await stepDefService.findAllStepDefsInternal({
                ...
                if (stepDefs.length > 0) {
                    ...
                    // TODO: better error handling
                    console.error(
                        `Error, Unable to locate the AWS Lambda step def on import for task def id ${newTaskDef._id}`
                    );
                }
            }
        } else {
            for (let stepDefToImport of taskDefToImport.stepDefs) {
                ...
                const newStepDef: StepDefSchema = await stepDefService.createStepDef(
                    _teamId,
                    newStepDefData,
                    correlationId
                );
