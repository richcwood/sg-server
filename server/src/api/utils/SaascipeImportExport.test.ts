...
describe('SaaScipe Script export/import tests', () => {
    ...
    test('Export/import saascipe script test', async () => {
        ...
        validateEquality(
            template['_publisherTeamId'].toHexString(),
            saascipeVersionMatch._publisherTeamId.toHexString()
        );
        validateEquality(
            template['_publisherUserId'].toHexString(),
            saascipeVersionMatch._publisherUserId.toHexString()
        );
...
describe('SaaScipe JobDef exporter tests', () => {
    ...
    test('Export saascipe JobDef test', async () => {
        ...
        for (let template of saascipeVersionTemplates) {
            ...
            validateEquality(
                Object.keys(jobDef.runtimeVars).length,
                Object.keys(saascipeDef.jobDef.runtimeVars).length
            );
            ...
            for (let schedule of schedules) {
                ...
                validateEquality(
                    importSchedule.FunctionKwargs.targetId.toHexString(),
                    importJobDefs[0]._id.toHexString()
                );
                for (let rtVar of Object.keys(schedule.FunctionKwargs.runtimeVars)) {
                    expect(Object.keys(importSchedule.FunctionKwargs.runtimeVars)).toEqual(
                        expect.arrayContaining([rtVar])
                    );
