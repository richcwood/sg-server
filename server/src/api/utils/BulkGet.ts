    ...
    for (let filterPart of inputFilterParts) {
        ...
        if (parts.length < 4) {
            throw new ValidationError(
                `The input filter '${inputFilter}" had a part that didn't appear to be well formed "${filterPart}'`
            );
        }
        ...
        switch (op) {
            ...
            case FilterOperator.GREATER_THAN_EQUAL_TO:
                ...
                if (isNaN(params)) {
                    throw new ValidationError(
                        `The input filter '${inputFilter}" had a ${op} operator that didn't have a valid number "${filterPart}'`
                    );
                }
                break;
            ...
            case FilterOperator.IN:
                try {
                    ...
                    throw new ValidationError(
                        `The input filter '${inputFilter}" had a -> filter didn't appear to be well formed "${filterPart}" params="${params}"`
                    );
                }
                break;
...
export const createQuery = function (
    schemaClass,
    modelClass,
    { filter = '', limit = 100, lastId, responseFields }: QueryOptions
) {
    let query;
...
export const defaultBulkGet = async function (
    filter: any,
    req: Request,
    resp: Response,
    next: NextFunction,
