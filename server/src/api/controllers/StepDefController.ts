...
export class StepDefController {
    ...
    //     return next(new MissingObjectError(`No step def found for task def ${_taskDefId}.`));
    //   }
    //   else {
    ...
    public async getStepDef(req: Request, resp: Response, next: NextFunction): Promise<void> {
        try {
            ...
            if (!stepDef) {
                return next(new MissingObjectError(`StepDef ${req.params.stepDefId} not found.`));
            } else {
            ...
            if (err instanceof Error.CastError) {
                if (req.params && req.params.stepDefId)
                    return next(new MissingObjectError(`StepDef ${req.params.stepDefId} not found.`));
                else return next(new MissingObjectError(`StepDef not found.`));
            } else {
                return next(err);
            }
        }
