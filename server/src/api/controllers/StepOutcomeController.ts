...
export class StepOutcomeController {
    ...
    //     return next(new MissingObjectError(`No step outcomes found for step ${_stepId}.`));
    //   }
    //   else {
    ...
    public async getStepOutcomesForTask(req: Request, resp: Response, next: NextFunction): Promise<void> {
        ...
        const steps = await stepOutcomeService.findStepOutcomesForTask(
            _teamId,
            _taskOutcomeId,
            null,
        ...
        if (_.isArray(steps) && steps.length === 0) {
            return next(new MissingObjectError(`No step outcomes found for task outcome ${_taskOutcomeId}.`));
        } else {
            response.data = convertResponseData(StepOutcomeSchema, steps);
            return next();
        }
    ...
    public async getStepOutcome(req: Request, resp: Response, next: NextFunction): Promise<void> {
        try {
            const _teamId: mongodb.ObjectId = new mongodb.ObjectId(<string>req.headers._teamid);
            const response: ResponseWrapper = (resp as any).body;
            const stepOutcome = await stepOutcomeService.findStepOutcome(
                _teamId,
                new mongodb.ObjectId(req.params.stepOutcomeId),
                <string>req.query.responseFields
            );
    ...
    public async createStepOutcome(req: Request, resp: Response, next: NextFunction): Promise<void> {
        ...
        const response: ResponseWrapper = resp['body'];
        try {
            const newStepOutcome = await stepOutcomeService.createStepOutcome(
                _teamId,
                convertRequestData(StepOutcomeSchema, req.body),
    ...
    public async updateStepOutcome(req: Request, resp: Response, next: NextFunction): Promise<void> {
        ...
        const response: ResponseWrapper = resp['body'];
        try {
            const updatedStepOutcome: any = await stepOutcomeService.updateStepOutcome(
                _teamId,
                new mongodb.ObjectId(req.params.stepOutcomeId),
                convertRequestData(StepOutcomeSchema, req.body),
                logger,
