...
export class TeamController {
  ...
  public async getTeam(req: Request, resp: Response, next: NextFunction): Promise<void> {
    try {
      ...
      const team = await teamService.findTeam(
        new mongodb.ObjectId(req.params.teamId),
  ...
  public async createUnassignedTeam(req: Request, resp: Response, next: NextFunction): Promise<void> {
    ...
    try {
      const newTeam: TeamSchema = <TeamSchema>(
        await teamService.createUnassignedTeam(
          convertRequestData(TeamSchema, req.body),
          logger,
          <string>req.query.responseFields
        )
      );
  ...
  public async createTeam(req: Request, resp: Response, next: NextFunction): Promise<void> {
    ...
    try {
      if (!req.headers.userid) {
        return next(new ValidationError(`Missing user id in header.`));
        return;
      }
      ...
      if (!user) {
        return next(new MissingObjectError(`User '${req.headers.userid}' not found.`));
        return;
      }

      req.body["ownerId"] = userId;
      const newTeam: TeamSchema = <TeamSchema>(
        await teamService.createTeam(convertRequestData(TeamSchema, req.body), logger, <string>req.query.responseFields)
      );
      response.data = convertResponseData(TeamSchema, newTeam);
      ...
      const userUpdated: UserSchema = <UserSchema>(
        await userService.updateUserInternal(
          userId,
          {teamIds: user.teamIds, teamAccessRightIds: user.teamAccessRightIds},
          logger
        )
      );
