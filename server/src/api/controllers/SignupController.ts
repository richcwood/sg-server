...
export class SignupController {
    ...
    public async confirmNewInvitedUser(req: Request, resp: Response, next: NextFunction): Promise<void> {
        ...
        const logger: BaseLogger = (<any>req).logger;
        try {
            const updatedUser: any = await signupService.confirmNewInvitedUser(
            ...
            if (_.isArray(updatedUser) && updatedUser.length === 0) {
                return next(new MissingObjectError(`User ${req.body.email} not found.`));
                return;
            }
    ...
    public async oauthSetup(req: Request, resp: Response, next: NextFunction): Promise<void> {
        const response: ResponseWrapper = resp['body'];
        try {
            const updatedUser: any = await signupService.oauthSetup(
    ...
    public async setInitialPassword(req: Request, resp: Response, next: NextFunction): Promise<void> {
        const response: ResponseWrapper = resp['body'];
        try {
            const updatedUser: any = await signupService.setInitialPassword(
                new mongodb.ObjectId(req.params.userId),
                req.body,
                <string>req.query.responseFields
            );
