...
export class TeamStorageController {
    ...
    public async getTeamStorage(req: Request, resp: Response, next: NextFunction): Promise<void> {
        try {
            ...
            const teamStorage = await teamStorageService.findTeamStorage(
                _teamId,
                new mongodb.ObjectId(req.params.teamStorageId),
                <string>req.query.responseFields
            );

            if (_.isArray(teamStorage) && teamStorage.length === 0) {
                return next(new MissingObjectError(`TeamStorage ${req.params.teamStorageId} not found.`));
            } else {
            ...
            if (err instanceof Error.CastError) {
                if (req.params && req.params.teamStorageId)
                    return next(new MissingObjectError(`TeamStorage ${req.params.teamStorageId} not found.`));
                else return next(new MissingObjectError(`TeamStorage not found.`));
            } else {
                return next(err);
            }
        }
    ...
    public async createTeamStorage(req: Request, resp: Response, next: NextFunction): Promise<void> {
        ...
        try {
            const newTeamStorage = await teamStorageService.createTeamStorage(
                _teamId,
                convertRequestData(TeamStorageSchema, req.body),
    ...
    public async updateTeamStorage(req: Request, resp: Response, next: NextFunction): Promise<void> {
        ...
        const response: ResponseWrapper = resp['body'];
        try {
            const updatedTeamStorage: any = await teamStorageService.updateTeamStorage(
                _teamId,
                new mongodb.ObjectId(req.params.teamStorageId),
                convertRequestData(TeamStorageSchema, req.body),
                req.header('correlationId'),
