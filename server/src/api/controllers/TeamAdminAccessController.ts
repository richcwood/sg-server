...
export class TeamAdminAccessController {
  ...
  public async grantTeamAdminAccess(req: Request, resp: Response, next: NextFunction): Promise<void> {
    ...
    try {
      ...
      const teamAdminAccessUpdated: any = await teamAdminAccessService.grantTeamAdminAccess(_teamId, new mongodb.ObjectId(req.params.userId), logger, req.header('correlationId'));
  ...
  public async revokeTeamAdminAccess(req: Request, resp: Response, next: NextFunction): Promise<void> {
    ...
    try {
      const logger: BaseLogger = (<any>req).logger;
      const _teamId: mongodb.ObjectId = new mongodb.ObjectId(<string>req.headers._teamid);
      const teamAdminAccessUpdated: any = await teamAdminAccessService.revokeTeamAdminAccess(_teamId, new mongodb.ObjectId(req.params.userId), logger, req.header('correlationId'));
