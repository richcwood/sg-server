...
export class TaskActionController {
    ...
    public async republishTask(req: Request, resp: Response, next: NextFunction): Promise<void> {
        try {
            ...
            const res = await taskActionService.republishTask(_teamId, new mongodb.ObjectId(req.params.taskId), logger, amqp);
            response.data = res;
    ...
    public async requeueTask(req: Request, resp: Response, next: NextFunction): Promise<void> {
        try {
            ...
            const _teamId: mongodb.ObjectId = new mongodb.ObjectId(<string>req.headers._teamid);
            const response: ResponseWrapper = resp['body'];
            const res = await taskActionService.requeueTask(_teamId, new mongodb.ObjectId(req.params.taskId), req.body, logger, amqp);
            response.data = res;
            response.statusCode = ResponseCode.OK;
            next();
        }
