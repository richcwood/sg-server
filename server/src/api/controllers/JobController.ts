...
export class JobController {
    ...
    public async createInteractiveConsoleJob(req: Request, resp: Response, next: NextFunction): Promise<void> {
        ...
        try {
            ...
            let newJob = await jobService.createJob(_teamId, req.body, createdBy, TaskSource.CONSOLE, logger, amqp, req.header('correlationId'), (<string>req.query.responseFields));
    ...
    public async createJob(req: Request, resp: Response, next: NextFunction): Promise<void> {
        ...
        try {
            ...
            if (Object.keys(req.headers).indexOf('_jobdefid') >= 0) {
                ...
                let newJob = await jobService.createJobFromJobDefId(_teamId, _jobDefId, req.body, logger, amqp, req.header('correlationId'), (<string>req.query.responseFields));
                response.data = convertResponseData(JobSchema, newJob);
                response.statusCode = ResponseCode.CREATED;
            } else if (Object.keys(req.headers).indexOf('jobdefname') >= 0) {
                const jobDefName: string = <string>req.headers.jobdefname;
                let newJob = await jobService.createJobFromJobDefName(_teamId, jobDefName, req.body, logger, amqp, req.header('correlationId'), (<string>req.query.responseFields));
                response.data = convertResponseData(JobSchema, newJob);
                response.statusCode = ResponseCode.CREATED;
            } else {
                let newJob = await jobService.createJob(_teamId, req.body, createdBy, TaskSource.JOB, logger, amqp, req.header('correlationId'), (<string>req.query.responseFields));
                response.data = convertResponseData(JobSchema, newJob);
                response.statusCode = ResponseCode.CREATED;
            }
            next();
    ...
    public async updateJob(req: Request, resp: Response, next: NextFunction): Promise<void> {
        ...
        const response: ResponseWrapper = resp['body'];
        try {
            let updatedJob: any = await jobService.updateJob(_teamId, new mongodb.ObjectId(req.params.jobId), convertRequestData(JobSchema, req.body), logger, amqp, req.header('correlationId'), (<string>req.query.responseFields));
