...
export class JoinTeamController {
  public async userJoinTeam(req: Request, resp: Response, next: NextFunction): Promise<void> {
    ...
    try {
      const user: any = await joinTeamService.userJoinTeam(
  ...
  public async anonymousJoinTeam(req: Request, resp: Response, next: NextFunction): Promise<void> {
    ...
    try {
      if (!req.headers.userid)
        return next(
          new ValidationError("Something went wrong. Please request a new invite link from the team administrator.")
        );
      const user: any = await joinTeamService.anonymousJoinTeam(
