...
export class TeamVariableController {
    ...
    public async getTeamVariable(req: Request, resp: Response, next: NextFunction): Promise<void> {
        try {
            ...
            const teamVariable = await teamVariableService.findTeamVariable(_teamId, new mongodb.ObjectId(req.params.teamVariableId), (<string>req.query.responseFields));

            if (_.isArray(teamVariable) && teamVariable.length === 0) {
                next(new MissingObjectError(`TeamVariable ${req.params.teamVariableId} not found.`));
            }
            else {
    ...
    public async createTeamVariable(req: Request, resp: Response, next: NextFunction): Promise<void> {
        ...
        try {
            const newTeamVariable = await teamVariableService.createTeamVariable(_teamId, convertRequestData(TeamVariableSchema, req.body), req.header('correlationId'), (<string>req.query.responseFields));
    ...
    public async updateTeamVariable(req: Request, resp: Response, next: NextFunction): Promise<void> {
        ...
        const response: ResponseWrapper = resp['body'];
        try {
            const updatedTeamVariable: any = await teamVariableService.updateTeamVariable(_teamId, new mongodb.ObjectId(req.params.teamVariableId), convertRequestData(TeamVariableSchema, req.body), req.header('correlationId'), (<string>req.query.responseFields));
    ...
    public async deleteTeamVariable(req: Request, resp: Response, next: NextFunction): Promise<void> {
        ...
        const response: ResponseWrapper = resp['body'];
        try {
            response.data = await teamVariableService.deleteTeamVariable(_teamId, new mongodb.ObjectId(req.params.teamVariableId), req.header('correlationId'));
            response.statusCode = ResponseCode.OK;
            next();
        }
        catch (err) {
