...
export class TaskOutcomeRouter {
  ...
  constructor() {
    ...
    this.router.get('/:taskOutcomeId', verifyAccessRights(['JOB_READ', 'GLOBAL']), taskOutcomeController.getTaskOutcome);
    this.router.post('/', verifyAccessRights(['TASK_OUTCOME_WRITE', 'GLOBAL']), taskOutcomeController.createTaskOutcome);
    this.router.put('/:taskOutcomeId', verifyAccessRights(['TASK_OUTCOME_WRITE', 'GLOBAL']), taskOutcomeController.updateTaskOutcome);
