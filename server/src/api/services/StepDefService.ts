...
export class StepDefService {
    ...
    public async findAllStepDefs(_orgId: mongodb.ObjectId, _taskDefId: mongodb.ObjectId, responseFields?: string) {
        return StepDefModel.find({ _taskDefId, _orgId }).select(responseFields);
    }


    public async findTaskDefStepDefs(_orgId: mongodb.ObjectId, _taskDefId: mongodb.ObjectId, responseFields?: string) {
        return StepDefModel.find({ _taskDefId, _orgId }).select(responseFields);
    }


    public async findStepDef(_orgId: mongodb.ObjectId, stepDefId: mongodb.ObjectId, responseFields?: string) {
        return StepDefModel.findById(stepDefId).find({ _orgId }).select(responseFields);
    }


    public async findStepDefByName(_orgId: mongodb.ObjectId, _taskDefId: mongodb.ObjectId, stepDefName: string, responseFields?: string) {
        let stepDef = await StepDefModel.find({ _orgId, _taskDefId, name: stepDefName }).select(responseFields);
    ...
    public async createStepDef(_orgId: mongodb.ObjectId, data: any, correlationId: string, responseFields?: string): Promise<object> {
        if (!data.name)
        ...
        const queryExistingStepDef = await this.findStepDefByName(_orgId, new mongodb.ObjectId(data._taskDefId), data.name, '_id');
        ...
        await rabbitMQPublisher.publish(_orgId, "StepDef", correlationId, PayloadOperation.CREATE, convertData(StepDefSchema, newStepDef));

        if (responseFields) {
            // It's is a bit wasteful to do another query but I can't chain a save with a select
            return this.findStepDef(_orgId, newStepDef._id, responseFields);
        }
        else {
    ...
    public async updateStepDef(_orgId: mongodb.ObjectId, id: mongodb.ObjectId, data: any, correlationId: string, responseFields?: string): Promise<object> {
        ...
        if (data['order'] !== undefined && data['order'] !== origStepDef.order) {
            const sameOrderStepDef = await StepDefModel.findOne({ _orgId, _taskDefId: origStepDef._taskDefId, order: data['order'] });
            ...
            if (!sameOrderStepDef) {
                ...
                const updatedSameOrder = await StepDefModel.findOneAndUpdate({ _id: sameOrderStepDef._id, _orgId }, { order: origStepDef.order }, { new: true });
                await rabbitMQPublisher.publish(_orgId, "StepDef", correlationId, PayloadOperation.UPDATE, convertData(StepDefSchema, updatedSameOrder));
            }
        }

        const updatedStepDef = await StepDefModel.findOneAndUpdate({ _id: id, _orgId }, data, { new: true }).select(responseFields);
        await rabbitMQPublisher.publish(_orgId, "StepDef", correlationId, PayloadOperation.UPDATE, convertData(StepDefSchema, updatedStepDef));

        return updatedStepDef; // fully populated model
    }


    public async deleteStepDef(_orgId: mongodb.ObjectId, id: mongodb.ObjectId, correlationId?: string): Promise<void> {
        const deleted = await StepDefModel.deleteOne({ _id: id });

        await rabbitMQPublisher.publish(_orgId, "StepDef", correlationId, PayloadOperation.DELETE, { id, correlationId });
