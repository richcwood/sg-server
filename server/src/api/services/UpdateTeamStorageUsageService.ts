...
export class UpdateTeamStorageUsageService {
    public async updateDailyTeamStorageUsage(
        ...
        if (!data.bytes) throw new MissingObjectError('Missing bytes');

        if (!_.isNumber(data.bytes)) throw new ValidationError(`bytes value "${data.bytes}" must be a valid number`);
        ...
        if (!_.isArray(teamStorageQuery) || teamStorageQuery.length === 0) {
            ...
            teamStorage = <TeamStorageSchema>(
                await teamStorageService.updateTeamStorage(_teamId, teamStorage._id, { bytes: newBytes })
            );
        }
