...
export class ArtifactService {
    ...
    public async createArtifact(_teamId: mongodb.ObjectId, data: any, correlationId: string): Promise<object> {
        ...
        await rabbitMQPublisher.publish(_teamId, "Artifact", correlationId, PayloadOperation.CREATE, convertData(ArtifactSchema, newArtifact));
    ...
    public async updateArtifact(_teamId: mongodb.ObjectId, id: mongodb.ObjectId, data: any, correlationId: string): Promise<object> {
        if (!data.name)
            throw new ValidationError(`Request body missing "name" parameter`);
        ...
        await rabbitMQPublisher.publish(_teamId, "Artifact", correlationId, PayloadOperation.UPDATE, convertData(ArtifactSchema, deltas));
