...
export class ArtifactService {
    ...
    public async findAllArtifacts(_orgId: mongodb.ObjectId, responseFields?: string) {
        return ArtifactModel.find({ _orgId }).select(responseFields);
    }


    public async findArtifact(_orgId: mongodb.ObjectId, id: mongodb.ObjectId) {
        const artifactQuery = await ArtifactModel.findById(id).find({ _orgId }).select('prefix name type');
    ...
    public async createArtifact(_orgId: mongodb.ObjectId, data: any, correlationId: string): Promise<object> {
        if (!data.name)
            throw new ValidationError(`Request body missing "name" parameter`);
        let filter = { _orgId, name: data.name };
        ...
        await FreeTierChecks.MaxArtifactStorageCheck(_orgId);
        ...
        if (_.isArray(existingArtifact) && existingArtifact.length > 0) {
            newArtifact = await this.updateArtifact(_orgId, existingArtifact[0]._id, data, correlationId);
    ...
    public async updateArtifact(_orgId: mongodb.ObjectId, id: mongodb.ObjectId, data: any, correlationId: string): Promise<object> {
        if (!data.name)
            throw new ValidationError(`Request body missing "name" parameter`);
        ...
        const artifactQuery = await ArtifactModel.findById(id).find({ _orgId }).select('prefix name type s3Path');
        if (!artifactQuery || (_.isArray(artifactQuery) && artifactQuery.length === 0))
            throw new MissingObjectError(`No artifact with id "${id.toHexString()}"`);
        const artifact: ArtifactSchema = artifactQuery[0];
        ...
        await rabbitMQPublisher.publish(_orgId, "Artifact", correlationId, PayloadOperation.UPDATE, convertData(ArtifactSchema, deltas));

        return updatedArtifact; // fully populated model
    }


    public async deleteArtifact(_orgId: mongodb.ObjectId, id: mongodb.ObjectId, correlationId: string): Promise<object> {
        const artifactQuery = await ArtifactModel.findById(id).find({ _orgId }).select('prefix name type, s3Path');
        if (!artifactQuery || (_.isArray(artifactQuery) && artifactQuery.length === 0))
            throw new MissingObjectError(`No artifact with id "${id.toHexString()}"`);
        const artifact: ArtifactSchema = artifactQuery[0];

        const tasksDefQuery: any = await taskDefService.findTaskDefs(_orgId, {artifacts: id.toHexString()}, "_id name");
        if (_.isArray(tasksDefQuery) && tasksDefQuery.length > 0) {
            ...
            throw new ValidationError(`Error deleting artifact - artifact is attached to the following tasks: ${tasksAsString}`);
        }
