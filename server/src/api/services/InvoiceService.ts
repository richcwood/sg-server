...
export class InvoiceService {
    ...
    public async findInvoice(
        ...
        invoiceId: mongodb.ObjectId,
        responseFields?: string
    ): Promise<InvoiceSchema | null> {
    ...
    public async updateInvoice(
        ...
        if ('_teamId' in data) throw new ValidationError('Unable to update _teamId field');
        if ('startDate' in data) throw new ValidationError('Unable to update startDate field');
        if ('endDate' in data) throw new ValidationError('Unable to update endDate field');
        if ('numScripts' in data) throw new ValidationError('Unable to update numScripts field');
        ...
        if (Object.keys(data).length > 0) {
            ...
            updatedInvoice = await InvoiceModel.findOneAndUpdate({ _id: id, _teamId }, data, { new: true }).select(
                responseFields
            );
        ...
        if (!updatedInvoice) throw new MissingObjectError(`Invoice with id '${id}' not found.`);
