...
export class TeamStorageService {
    ...
    public async createTeamStorage(_teamId: mongodb.ObjectId, data: any, correlationId?: string, responseFields?: string): Promise<object> {
        if (!data.date)
    ...
    public async updateTeamStorage(_teamId: mongodb.ObjectId, id: mongodb.ObjectId, data: any, correlationId?: string, responseFields?: string): Promise<object> {
        const filter = { _id: id, _teamId };
        data = Object.assign(data, { $inc: { 'numobservations': 1 } });
        const updatedTeamStorage = await TeamStorageModel.findOneAndUpdate(filter, data, { new: true }).select(responseFields);

        if (!updatedTeamStorage)
            throw new MissingObjectError(`TeamStorage '${id}" not found with filter "${JSON.stringify(filter, null, 4)}'.`)
