...
export class PaymentTransactionService {
    ...
    public async findAllPaymentTransactions(_orgId: mongodb.ObjectId, responseFields?: string) {
        return PaymentTransactionModel.find({ _orgId }).select(responseFields);
    }


    public async findPaymentTransaction(_orgId: mongodb.ObjectId, paymentTransactionId: mongodb.ObjectId, responseFields?: string) {
        return PaymentTransactionModel.findById(paymentTransactionId).find({ _orgId }).select(responseFields);
    }
    ...
    public async createPaymentTransaction(_orgId: mongodb.ObjectId, data: any, correlationId: string, responseFields?: string): Promise<object> {
        data._orgId = _orgId;
        ...
        await rabbitMQPublisher.publish(_orgId, "PaymentTransaction", correlationId, PayloadOperation.CREATE, convertData(PaymentTransactionSchema, newPaymentTransaction));

        if (responseFields) {
            // It's is a bit wasteful to do another query but I can't chain a save with a select
            return this.findPaymentTransaction(_orgId, newPaymentTransaction.id, responseFields);
        }
        else {
    ...
    public async updatePaymentTransaction(_orgId: mongodb.ObjectId, id: mongodb.ObjectId, data: any, correlationId: string, responseFields?: string): Promise<object> {
        ...
        await rabbitMQPublisher.publish(_orgId, "PaymentTransaction", correlationId, PayloadOperation.UPDATE, convertData(PaymentTransactionSchema, deltas));
