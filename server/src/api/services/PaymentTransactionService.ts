...
export class PaymentTransactionService {
    ...
    public async findPaymentTransaction(
        _teamId: mongodb.ObjectId,
        paymentTransactionId: mongodb.ObjectId,
    ...
    public async updatePaymentTransaction(
        ...
        if ('_invoiceId' in data) throw new ValidationError('Unable to update _invoiceId field');
        if ('source' in data) throw new ValidationError('Unable to update source field');
        if ('transactionId' in data) throw new ValidationError('Unable to update transactionId field');
        if ('amount' in data) throw new ValidationError('Unable to update amount field');

        const filter = { _id: id, _teamId };
        const updatedPaymentTransaction = await PaymentTransactionModel.findOneAndUpdate(filter, data, {
            new: true,
        }).select(responseFields);

        if (!updatedPaymentTransaction)
            throw new MissingObjectError(
                `PaymentTransaction '${id}" not found with filter "${JSON.stringify(filter, null, 4)}'.`
            );
        ...
        await rabbitMQPublisher.publish(
            ...
            convertData(PaymentTransactionSchema, deltas)
        );
