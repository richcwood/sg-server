...
export class TaskDefService {
    ...
    public async createTaskDef(
        ...
        for (let i = 0; i < taskDefs.length; i++) {
            ...
            if (taskDef.name == data.name)
                throw new ValidationError(
                    `Job template "${data._jobDefId}" already contains a task with name "${data.name}"`
                );
        }
        ...
        if (Object.keys(cd).length > 0)
            throw new ValidationError(
                `New task would create cyclic dependency with the following tasks: ${Object.keys(cd).filter(
                    (key) => cd[key]
                )}`
            );
            ...
            if (name_updated != taskName_original) {
                for (let i = 0; i < taskDefs.length; i++) {
                    ...
                    if (needToUpdate) {
                        ...
                        await TaskDefModel.findOneAndUpdate({ _id: taskDefs[i]._id }, taskRoutesUpdate, {
                            new: true,
    ...
    public async deleteTaskDef(
        ...
        await rabbitMQPublisher.publish(_teamId, 'TaskDef', correlationId, PayloadOperation.DELETE, {
            id,
            correlationId,
        });
