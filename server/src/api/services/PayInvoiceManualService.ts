...
export class PayInvoiceManualService {
    public async payInvoice(
        ...
        if (!data.amount) throw new ValidationError(`Request body missing "amount" parameter`);

        if (!data.paymentMethodId) throw new ValidationError(`Request body missing "paymentMethodId" parameter`);
        ...
        try {
            ...
            if (paymentIntent.status != 'succeeded') throw new Error('Manual payment processing failed');
            ...
            if (invoice.paidAmount + amount >= invoice.billAmount) queryUpdateInvoice['status'] = InvoiceStatus.PAID;
            else queryUpdateInvoice['status'] = InvoiceStatus.PARTIALLY_PAID;
