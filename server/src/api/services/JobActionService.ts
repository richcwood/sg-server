...
export class JobActionService {
    private async interruptJobTasks(_teamId: mongodb.ObjectId, _jobId: mongodb.ObjectId, logger: BaseLogger) {
        ...
        const tasksToInterruptQuery = await taskService.findAllTasksInternal({
            filter: tasksToInterruptFilter,
            responseFields: '_id',
        });
        ...
        const taskOutcomesToInterruptQuery = await taskOutcomeService.findAllTaskOutcomesInternal(
            taskOutcomesToInterruptFilter,
            '_id'
        );
    ...
    private async cancelJobTasks(_teamId: mongodb.ObjectId, _jobId: mongodb.ObjectId, logger: BaseLogger) {
        ...
        const tasksToCancelQuery = await taskService.findAllTasksInternal({
            filter: tasksToCancelFilter,
            responseFields: '_id',
        });
        ...
        const taskOutcomesToCancelQuery = await taskOutcomeService.findAllTaskOutcomesInternal(
            taskOutcomesToCancelFilter,
            '_id'
        );
    ...
    private async restartJobTasks(
        ...
        taskOutcomesToRestartFilter['$or'] = [
        ...
        const taskOutcomesToRestartQuery = await taskOutcomeService.findAllTaskOutcomesInternal(
            taskOutcomesToRestartFilter,
            '_id _taskId'
        );
        ...
        // const tasksToRestartQuery = await taskService.findAllTasksInternal({
        //   filter: tasksToRestartFilter,
        //   responseFields: "_id",
        // });
    ...
    public async interruptJob(
        ...
        let updatedJob: JobSchema = await JobModel.findOneAndUpdate(
            filter,
        ...
        if (updatedJob)
            await rabbitMQPublisher.publish(
                ...
                convertData(JobSchema, updatedJob)
            );
        ...
        if (!updatedJob) {
            ...
            throw new ValidationError(
                `Job ${_jobId} cannot be interrupted - current status should be "RUNNING" but is "${
                    JobStatus[jobQuery[0].status]
                }"`
            );
        }

        if (jobInterrupted) await this.interruptJobTasks(_teamId, _jobId, logger);
    ...
    public async restartJob(
        ...
        if (!updatedJob) {
            ...
            updatedJob = jobQuery[0];
            throw new ValidationError(
                `Job ${_jobId} cannot be restarted since current status is "${JobStatus[jobQuery[0].status]}"`
            );
        }

        if (jobRestarted) await this.restartJobTasks(_teamId, _jobId, logger, amqp);
    ...
    public async cancelJob(
        ...
        const filter = {
            _id: _jobId,
            _teamId,
            $or: [{ status: { $lt: JobStatus.CANCELING } }, { status: JobStatus.FAILED }],
        };
        let updatedJob: JobSchema = await JobModel.findOneAndUpdate(
            filter,
        ...
        if (!updatedJob) {
            ...
            updatedJob = jobQuery[0];
            throw new ValidationError(
                `Job ${_jobId} cannot be canceled since current status is "${JobStatus[jobQuery[0].status]}"`
            );
        }

        if (jobCanceled) await this.cancelJobTasks(_teamId, _jobId, logger);
