...
export class UserScriptShadowCopyService {
    ...
    public async findUserScriptShadowCopy(
        _teamId: mongodb.ObjectId,
        userScriptShadowCopyId: mongodb.ObjectId,
    ...
    public async createUserScriptShadowCopy(
        _teamId: mongodb.ObjectId,
        data: any,
        ...
        await rabbitMQPublisher.publish(
            _teamId,
            'UserScriptShadowCopy',
            correlationId,
            PayloadOperation.CREATE,
            convertData(UserScriptShadowCopySchema, newUserScriptShadowCopy)
        );
    ...
    public async updateUserScriptShadowCopy(
        _teamId: mongodb.ObjectId,
        id: mongodb.ObjectId,
        data: any,
        correlationId: string,
        ...
        const updatedUserScriptShadowCopy = await UserScriptShadowCopyModel.findOneAndUpdate(filter, data, {
            new: true,
        }).select(responseFields);

        if (!updatedUserScriptShadowCopy)
            throw new MissingObjectError(
                `UserScriptShadowCopy '${id}" not found with filter "${JSON.stringify(filter, null, 4)}'.`
            );
        ...
        await rabbitMQPublisher.publish(
            ...
            convertData(UserScriptShadowCopySchema, deltas)
        );

        return updatedUserScriptShadowCopy; // fully populated model
    }

    public async deleteUserScriptShadowCopy(
        _teamId: mongodb.ObjectId,
        id: mongodb.ObjectId,
        correlationId: string
    ): Promise<object> {
        ...
        await rabbitMQPublisher.publish(_teamId, 'UserScriptShadowCopy', correlationId, PayloadOperation.DELETE, {
            _id: id,
        });
