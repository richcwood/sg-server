...
export class TeamVariableService {
    ...
    public async findTeamVariable(
        _teamId: mongodb.ObjectId,
        teamVariableId: mongodb.ObjectId,
    ...
    public async createTeamVariable(
        _teamId: mongodb.ObjectId,
        data: any,
        ...
        if (!data.value) throw new ValidationError(`Request body missing "value" parameter`);
        ...
        await rabbitMQPublisher.publish(
            ...
            convertData(TeamVariableSchema, newTeamVariable)
        );
    ...
    public async updateTeamVariable(
        _teamId: mongodb.ObjectId,
        id: mongodb.ObjectId,
        data: any,
        correlationId: string,
        ...
        const updatedTeamVariable = await TeamVariableModel.findOneAndUpdate(filter, data, { new: true }).select(
            responseFields
        );

        if (!updatedTeamVariable)
            throw new MissingObjectError(
                `TeamVariable '${id}" not found with filter "${JSON.stringify(filter, null, 4)}'.`
            );
        ...
        await rabbitMQPublisher.publish(
            ...
            convertData(TeamVariableSchema, deltas)
        );

        return updatedTeamVariable; // fully populated model
    }

    public async deleteTeamVariable(
        _teamId: mongodb.ObjectId,
        id: mongodb.ObjectId,
        correlationId: string
    ): Promise<object> {
