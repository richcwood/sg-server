...
export class TaskOutcomeService {
    ...
    public async updateTaskOutcome(
        ...
        try {
            ...
            if (
                data.status &&
                data.status != currentTaskOutcome.status &&
                currentTaskOutcome.status >= TaskStatus.SUCCEEDED
            )
            ...
            let updatedTaskOutcome: TaskOutcomeSchema = await TaskOutcomeModel.findOneAndUpdate(filter, data, {
                new: true,
            });
            ...
            if (data.status && data.status != currentTaskOutcome.status) {
                if (data.status >= TaskStatus.SUCCEEDED || data.status == TaskStatus.INTERRUPTED) {
                    if (updatedTaskOutcome.runtimeVars) {
                            ...
                            if (key != SGStrings.route) {
                                ...
                                if (!resJobQuery)
                                    throw new MissingObjectError(
                                        `Job '${updatedTaskOutcome._jobId}" not found for team "${_teamId}"`
                                    );
                            }
                        }
                        ...
                        if (!resTaskQuery)
                            throw new MissingObjectError(
                                `Tasl '${updatedTaskOutcome._taskId}" not found for team "${_teamId}"`
                            );
                    }
                }
                ...
                if (
                    ...
                    (currentTaskOutcome.status == TaskStatus.CANCELLED ||
                        currentTaskOutcome.status == TaskStatus.CANCELING)
                ) {
                ...
                if (
                    updatedTaskOutcome.status >= TaskStatus.SUCCEEDED ||
            ...
            for (let i = 0; i < task.toRoutes.length; i++) {
                ...
                if (route.search(routePattern) >= 0) {
                    ...
                    if (downstreamTask) {
                        ...
                        logger.LogError(
                            `Unable to find Task with _jobId "${task._jobId}" and name "${downStreamTaskName}"`,
                            {
                                Class: 'TaskOutcomeService',
                ...
                if (job.status == JobStatus.CANCELING || job.status == JobStatus.COMPLETED) {
                    ...
                    if (route.search(routePattern) >= 0) {
                        ...
                        if (downstreamTask) {
                            ...
                            downstreamTask = await TaskModel.findOneAndUpdate(taskFilter, {
                                $unset: queryUpdate,
                            }).select('_id up_dep');
                            if (
                    ...
                    if (arrFindVarsArgs) {
                        ...
                        for (let i = 0; i < arrFindVarsArgs.length; i++) {
                            try {
                                ...
                                logger.LogError(
                                    `Error in arguments @sgg capture for string \"${arrFindVarsArgs[i]}\": ${e.message}`,
                                    {
                                        Class: 'TaskOutcomeService',
                                        Method: 'PublishTask',
                                        _teamId,
    ...
    async OnTaskSkipped(_teamId: mongodb.ObjectId, _jobId: mongodb.ObjectId, taskName: string, logger: BaseLogger) {
        return new Promise<void>(async (resolve, reject) => {
            ...
            try {
                ...
                if (!task) {
                    logger.LogError(`Error in OnTaskSkipped - task not found`, {
                        _teamId,
                        _jobId: _jobId,
                        taskName: taskName,
                    });
