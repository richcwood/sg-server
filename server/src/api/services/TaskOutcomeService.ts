...
export class TaskOutcomeService {
    ...
    public async findTaskOutcomes(_orgId: mongodb.ObjectId, filter: any, responseFields?: string) {
        filter = Object.assign({ _orgId }, filter);
        return TaskOutcomeModel.find(filter).select(responseFields);
    }


    public async findTaskOutcome(_orgId: mongodb.ObjectId, taskOutcomeId: mongodb.ObjectId, responseFields?: string) {
        return TaskOutcomeModel.findById(taskOutcomeId).find({ _orgId }).select(responseFields);
    }
    ...
    public async createTaskOutcome(_orgId: mongodb.ObjectId, data: any, logger: BaseLogger, correlationId?: string, responseFields?: string): Promise<object> {
        data._orgId = _orgId;
        const taskOutcomeModel = new TaskOutcomeModel(data);
        let newTaskOutcome = await taskOutcomeModel.save();

        await rabbitMQPublisher.publish(_orgId, "TaskOutcome", correlationId, PayloadOperation.CREATE, convertData(TaskOutcomeSchema, newTaskOutcome));

        // if (newTaskOutcome.status >= TaskStatus.SUCCEEDED || newTaskOutcome.status == TaskStatus.INTERRUPTED) {
        //     await this.updateTaskOutcome(_orgId, newTaskOutcome._id, newTaskOutcome, logger);
        // }

        const taskQuery: TaskSchema[] = await taskService.findTask(_orgId, newTaskOutcome._taskId, '_id status');
        if ((_.isArray(taskQuery) && taskQuery.length > 0)) {
            if (taskQuery[0].status == TaskStatus.INTERRUPTED) {
                newTaskOutcome = await taskOutcomeActionService.interruptTaskOutcome(_orgId, newTaskOutcome._id);
            } else if (taskQuery[0].status == TaskStatus.CANCELLED) {
                newTaskOutcome = await taskOutcomeActionService.cancelTaskOutcome(_orgId, newTaskOutcome._id);
            }
        }

        await jobService.UpdateJobStatus(_orgId, newTaskOutcome._jobId, logger, null);

        if (responseFields) {
            return this.findTaskOutcome(_orgId, newTaskOutcome._id, responseFields);
        }
        else {
    ...
    public async updateTaskOutcome(_orgId: mongodb.ObjectId, id: mongodb.ObjectId, data: any, logger: BaseLogger, filter?: any, correlationId?: string, responseFields?: string): Promise<object> {
         try {
            const currentTaskOutcomeQuery: TaskOutcomeSchema[] = await this.findTaskOutcome(_orgId, id, 'status');
            ...
            if (!currentTaskOutcomeQuery || (_.isArray(currentTaskOutcomeQuery) && currentTaskOutcomeQuery.length === 0))
                throw new MissingObjectError(`TaskOutcome "${id}" not found for org "${_orgId}"`);
            ...
            if (data.status && data.status != currentTaskOutcome.status) {
                    ...
                    if (updatedTaskOutcome.runtimeVars) {
                        for (let i = 0; i < Object.keys(updatedTaskOutcome.runtimeVars).length; i++) {
                            ...
                            if (key != KikiStrings.route) {
                                ...
                                const resJobQuery = await jobService.updateJob(_orgId, updatedTaskOutcome._jobId, qryUpdate);
                                if (!resJobQuery)
                                    throw new MissingObjectError(`Job '${updatedTaskOutcome._jobId}" not found for org "${_orgId}"`);
                            }
                        }

                        const resTaskQuery = await taskService.updateTask(_orgId, updatedTaskOutcome._taskId, { runtimeVars: updatedTaskOutcome.runtimeVars }, logger);
                        if (!resTaskQuery)
                            throw new MissingObjectError(`Tasl '${updatedTaskOutcome._taskId}" not found for org "${_orgId}"`);
                    }
                }
                ...
                if (data.status == TaskStatus.INTERRUPTED && (currentTaskOutcome.status == TaskStatus.INTERRUPTING)) {
                    const taskStepOutcomesQuery = await stepOutcomeService.findStepOutcomesForTask(_orgId, id, null, '_id');
                    if (_.isArray(taskStepOutcomesQuery) && taskStepOutcomesQuery.length > 0) {
                        for (let i = 0; i < taskStepOutcomesQuery.length; i++) {
                            ...
                            await StepOutcomeModel.findOneAndUpdate({ _id: stepOutcome._id, _orgId, status: { $lt: StepStatus.INTERRUPTED } }, stepOutcomeUpdate);
                            // await stepOutcomeService.updateStepOutcome(_orgId, stepOutcome._id, stepOutcomeUpdate, null, { status: { $lt: StepStatus.INTERRUPTED } }, null, '_id');
                        }
                    }
                }
                ...
                if (data.status == TaskStatus.INTERRUPTED && (currentTaskOutcome.status == TaskStatus.CANCELLED || currentTaskOutcome.status == TaskStatus.CANCELING)) {
                    ...
                    if (_.isArray(taskStepOutcomesQuery) && taskStepOutcomesQuery.length > 0) {
                        for (let i = 0; i < taskStepOutcomesQuery.length; i++) {
                            const stepOutcome: any = taskStepOutcomesQuery[i];
                            await StepOutcomeModel.findOneAndUpdate({ _id: stepOutcome._id, _orgId, status: { $lte: StepStatus.INTERRUPTED } }, { status: StepStatus.INTERRUPTED });
                            // await stepOutcomeService.updateStepOutcome(_orgId, stepOutcome._id, { status: StepStatus.CANCELLED }, null, { status: { $lte: StepStatus.INTERRUPTED } }, null, '_id');
                        }
                    }
                }

                if (updatedTaskOutcome.status >= TaskStatus.SUCCEEDED || updatedTaskOutcome.status == TaskStatus.INTERRUPTED) {
                    const job: JobSchema = await jobService.UpdateJobStatus(_orgId, updatedTaskOutcome._jobId, logger, null, 'name runId onJobTaskInterruptedAlertEmail onJobTaskInterruptedAlertSlackURL');

                    if (job.status != Enums.JobStatus.INTERRUPTING && job.status != Enums.JobStatus.INTERRUPTED)
                        await this.LaunchDownstreamTasks(_orgId, job, updatedTaskOutcome, logger);

                    if (job._jobDefId) {
                        if (job.status == Enums.JobStatus.INTERRUPTED || job.status == Enums.JobStatus.FAILED) {
                            try {
                                await jobDefService.updateJobDef(_orgId, job._jobDefId, { status: Enums.JobDefStatus.PAUSED }, { status: Enums.JobDefStatus.RUNNING, pauseOnFailedJob: true });
                            } catch (e) {
                        ...
                        jobService.LaunchReadyJobs(_orgId, job._jobDefId);
                    }
                    ...
                    if (updatedTaskOutcome.status == Enums.TaskStatus.INTERRUPTED) {
                        const taskQuery: TaskSchema[] = await taskService.findTask(_orgId, updatedTaskOutcome._taskId, '_id autoRestart');
                        if ((_.isArray(taskQuery) && taskQuery.length > 0)) {
                            if (taskQuery[0].autoRestart) {
                                ...
                                if (updatedTaskOutcome)
                                    data.status = updatedTaskOutcome.status;
                                await localRestAccess.RestAPICall(`taskaction/republish/${taskQuery[0]._id}`, 'POST', _orgId, null, null);
                            }
                        }
                    }
                }
            ...
            await jobService.UpdateJobStatus(_orgId, updatedTaskOutcome._jobId, logger, null);

            return updatedTaskOutcome; // fully populated model
        } catch (err) {
            logger.LogError(err, { Class: 'TaskOutcomeService', Method: 'updateTaskOutcome', _orgId, taskOutcome: data });
            throw err;
        }
    }


    async LaunchDownstreamTasks(_orgId: mongodb.ObjectId, job: JobSchema, taskOutcome: TaskOutcomeSchema, logger: BaseLogger) {
        let tasksToLaunch = [];
        let toRouteTasks = [];

        const filter: any = { _orgId, _id: taskOutcome._taskId };
        ...
        if (task.toRoutes && job.status != Enums.JobStatus.CANCELING && job.status != Enums.JobStatus.COMPLETED) {
            for (let i = 0; i < task.toRoutes.length; i++) {
                ...
                if (route.search(routePattern) >= 0) {
                    let queryFindRes = await taskService.findTaskByName(_orgId, task._jobId, downStreamTaskName, 'id');
                    if (_.isArray(queryFindRes) && queryFindRes.length > 0) {
                        ...
                        logger.LogError(`Unable to find Task with _jobId "${task._jobId}" and name "${downStreamTaskName}"`, { Class: 'TaskOutcomeService', Method: 'LaunchDownstreamTasks', _orgId, task: task });
                    }
                }
                // else if (route != 'fail') {
                //     await this.OnTaskSkipped(_orgId, task._jobId, downStreamTaskName, logger);
                // }
            }
        }
        ...
        if (task.down_dep) {
            for (let i = 0; i < task.down_dep.length; i++) {
                ...
                if (job.status == Enums.JobStatus.CANCELING || job.status == Enums.JobStatus.COMPLETED) {
                    ...
                    if (route.search(routePattern) >= 0) {
                        let queryFindRes = await taskService.findTaskByName(_orgId, taskOutcome._jobId, downStreamTaskName, 'id');
                        if (_.isArray(queryFindRes) && queryFindRes.length > 0) {
                            let downstreamTask: TaskSchema = queryFindRes[0];
                            let queryUpdate: any = {};
                            queryUpdate[`up_dep.${task.name}`] = '';
                            const taskFilter = { _orgId, _id: downstreamTask.id };
                            ...
                            logger.LogError(`Unable to find Task with _jobId "${task._jobId}" and name "${downStreamTaskName}"`, { Class: 'TaskOutcomeService', Method: 'LaunchDownstreamTasks', _orgId, _jobId: taskOutcome._jobId, taskOutcome: taskOutcome });
                        }
                    } else if (route != 'fail') {
                        await this.OnTaskSkipped(_orgId, task._jobId, downStreamTaskName, logger);
                    }
        ...
        for (let i = 0; i < tasksToLaunch.length; i++) {
            await this.LaunchTask(_orgId, tasksToLaunch[i], logger);
        }
    }


    async LaunchTask(_orgId: mongodb.ObjectId, taskToLaunch: any, logger: BaseLogger) {
        ...
        try {
            if (taskToLaunch.sourceTaskRoute) {
                const taskQuery = await taskService.findTask(_orgId, _taskId);
                if (!taskQuery || (_.isArray(taskQuery) && taskQuery.length == 0)) {
                    logger.LogError(`No task found with id ${_taskId}`, { Class: 'TaskOutcomeService', Method: 'LaunchTask', _orgId, Task: task });
                    return;
                } else {
                    task = taskQuery[0];
                ...
                task = await taskService.updateTask(_orgId, _taskId, { status: TaskStatus.NOT_STARTED }, logger, { status: null }, null, null);
            }
            await this.PublishTask(_orgId, task, logger, amqp);
        } catch (err) {
            if (err instanceof MissingObjectError) {
                logger.LogWarning(err.message, { Error: err, Class: 'TaskOutcomeService', Method: 'LaunchTask' });
            } else {
                logger.LogError(err.message, { Error: err, Class: 'TaskOutcomeService', Method: 'LaunchTask', _orgId, Task: task });
            }
        }
    }


    async PublishTask(_orgId: mongodb.ObjectId, task: TaskSchema, logger: BaseLogger, amqp: AMQPConnector) {
        let success: boolean = false;
        try {
            let getTaskRoutesRes = await GetTaskRoutes(_orgId, task, logger);
            ...
            if (!getTaskRoutesRes.routes) {
                ...
                if (getTaskRoutesRes.failureCode == Enums.TaskFailureCode.TARGET_AGENT_NOT_SPECIFIED) {
                    ...
                    await taskService.updateTask(_orgId, task._id, deltas, logger)
                ...
                // await rabbitMQPublisher.publish(_orgId, "Task", null, PayloadOperation.UPDATE, convertData(TaskOutcomeSchema, deltas));
                ...
                const resFindJob = await jobService.findJob(_orgId, task._jobId, 'runtimeVars');
                ...
                for (let step of await stepService.findAllTaskSteps(_orgId, task._id)) {
                    if (step.variables) {
                        for (let e = 0; e < Object.keys(step.variables).length; e++) {
                            ...
                            if (!runtimeVarsTask[eKey]) {
                                if (job.runtimeVars[eKey]) {
                                    ...
                                    const orgVar = await orgVariableService.findOrgVariableByName(_orgId, eKey, 'value');
                                    if (_.isArray(orgVar) && orgVar.length > 0)
                                        runtimeVarsTask[eKey] = orgVar[0].value;
                                }
                            }
                        }
                    }
                            ...
                            try {
                                ...
                                if (!runtimeVarsTask[varKey]) {
                                    if (job.runtimeVars[varKey]) {
                                        ...
                                        if (_.isArray(orgVar) && orgVar.length > 0)
                                            runtimeVarsTask[varKey] = orgVar[0].value;
                                    }
                                }
                            } catch (e) {
                                logger.LogError(`Error in arguments @kpg capture for string \"${arrFindVarsArgs[i]}\": ${e.message}`, { Class: 'JobRouter', Method: 'PublishTask', _orgId, task: task });
                            }
                        }
                    }
                ...
                await taskService.updateTask(_orgId, task._id, { status: Enums.TaskStatus.PUBLISHED, route: '', failureCode: '' }, logger);
                success = true;
            }

            await jobService.UpdateJobStatus(_orgId, task._jobId, logger, null);
            return { success };
        } catch (err) {
    ...
    async OnTaskSkipped(_orgId: mongodb.ObjectId, _jobId: mongodb.ObjectId, taskName: string, logger: BaseLogger) {
        return new Promise(async (resolve, reject) => {
            let resUpdate;

            try {
                const tasks: TaskSchema[] = await taskService.findTaskByName(_orgId, _jobId, taskName);
                if (!tasks || tasks.length < 1) {
                    logger.LogError(`Error in OnTaskSkipped - task not found`, { _orgId, _jobId: _jobId, taskName: taskName });
                    resolve();
                    return;
                }
                ...
                resUpdate = await taskService.updateTask(_orgId, task.id, { status: Enums.TaskStatus.SKIPPED }, logger);
                ...
                if (task.down_dep) {
                    ...
                    for (let i = 0; i < dependentTask.length; i++) {
                        await this.OnTaskSkipped(_orgId, _jobId, dependentTask[i], logger);
                    }
                }
