...
export class JoinOrgService {
    public async userJoinOrg(_userId: mongodb.ObjectId, _orgId: string, token: string): Promise<object> {
        /// Check if the invited user exists
        const userModel: any = await userService.findUser(_userId, '_id email orgIds orgIdsInvited orgIdsInactive');
        if (!userModel)
        ...
        let orgIdInviteSecret: string = undefined;
        if (_.isArray(filterRes) && filterRes.length > 0)
            orgIdInviteSecret = filterRes[0].inviteKey;
        if (!orgIdInviteSecret) {
            if (!userAlreadyInOrg)
                throw new ValidationError('Something went wrong. Please request a new invite from the team administrator.');
        } else {
            ...
            let jwtData;
            try {
                jwtData = jwt.verify(token, orgIdInviteSecret);
            } catch (err) {
                tokenIsValid = false;
            }
            if (tokenIsValid) {
                if (_userId.toHexString() != jwtData.id)
                    tokenIsValid = false;
                else if (_orgId != jwtData._orgId)
                    tokenIsValid = false;
            }
        }

        /// If the user is already in the org it doesn't matter if the token is valid or not - just remove the secret
        ///     and if somehow the org was added to the inactive list, remove it
        if (userAlreadyInOrg) {
    ...
    public async anonymousJoinOrg(_userId: mongodb.ObjectId, token: string): Promise<object> {
        /// Check if the invited user exists
        const userModel: any = await userService.findUser(_userId, '_id email name companyName emailConfirmed orgIds orgIdsInvited orgIdsInactive');
        ...
        /// Check if the user is already in the org
        let userAlreadyInOrg: boolean = true;
        if (userModel.orgIds.indexOf(jwtData.InvitedOrgId) < 0)
            userAlreadyInOrg = false;

            console.log('\n\nHELLO userAlreadyInOrg -> ', userAlreadyInOrg);
        /// If the user is already in the org it doesn't matter if the token is valid or not - just remove the secret
        ///     and if somehow the org was added to the inactive list, remove it
        if (userAlreadyInOrg) {
            return userModel;
        ...
        let newOrgIdsInvited: any[] = [];
        for (let i = 0; i < userModel.orgIdsInvited.length; i++) {
            if (userModel.orgIdsInvited[i]._orgId != jwtData.InvitedOrgId) {
                newOrgIdsInvited.push(userModel.orgIdsInvited[i]);
            }
        }
        console.log('\n\nHELLO bbb -> ');
    ...
    // public async anonymousJoinOrg(_orgId: string, token: string): Promise<object> {
    //     /// Check if the invited user exists
    //     const orgModel: any = await orgService.findOrg(new mongodb.ObjectId(_orgId), '_id name');
    //     if (!orgModel)
...
export const joinOrgService = new JoinOrgService();