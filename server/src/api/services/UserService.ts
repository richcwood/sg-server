...
export class UserService {
    ...
    public async updateUserInternal(
        id: mongodb.ObjectId,
        data: any,
        logger: BaseLogger,
        ...
        if (!user.emailConfirmed) throw new ValidationError('Email address not verified');

        if (!user.hasAcceptedTerms) throw new ValidationError('User has not accepted terms');
        ...
        if (data.teamIds) {
            updatedUser = await UserModel.findOneAndUpdate(filter, { teamIds: data.teamIds }, { new: true }).select(
                responseFields
            );
        }
        ...
        if (data.teamIdsInvited) {
            updatedUser = await UserModel.findOneAndUpdate(
                filter,
                { teamIdsInvited: data.teamIdsInvited },
                { new: true }
        ...
        if (data.teamAccessRightIds) {
            updatedUser = await UserModel.findOneAndUpdate(
                filter,
                { teamAccessRightIds: data.teamAccessRightIds },
                { new: true }
            ).select(responseFields);
        }
        ...
        if (data.teamIdsInactive) {
            updatedUser = await UserModel.findOneAndUpdate(
                filter,
                { teamIdsInactive: data.teamIdsInactive },
                { new: true }
            ).select(responseFields);
        }
    ...
    public async leaveTeam(
        ...
        if (!userModel) throw new MissingObjectError('User not found');

        if (!userModel.teamIds) return userModel;

        /// Check if the user is in the team
        if (userModel.teamIds.indexOf(teamIdAsString) < 0) {
            logger.LogError('User is not a member of the team', {
                Class: 'UserService',
                Method: 'leaveTeam',
        ...
        for (let i = 0; i < userModel.teamIds.length; i++) {
            if (userModel.teamIds[i] != teamIdAsString) newTeamIds.push(userModel.teamIds[i]);
            else newTeamIdsInactive.push(userModel.teamIds[i]);
        }
        ...
        const updatedUser = await UserModel.findOneAndUpdate(
            filter,
            { teamIds: newTeamIds, teamIdsInactive: newTeamIdsInactive, teamAccessRightIds: newTeamAccessRightIds },
            { new: true }
        ).select('_id teamIds teamIdsInactive');

        logger.LogWarning('User removed from team', {
            Class: 'UserService',
            Method: 'leaveTeam',
            TeamId: teamIdAsString,
            id: id.toHexString(),
            RemovedBy: removedById.toHexString(),
        });
    ...
    public async joinTeam(id: mongodb.ObjectId, _teamId: string): Promise<object> {
        ...
        if (_.isArray(filterRes) && filterRes.length > 0) userInvitedToTeam = true;

        if (!userInvitedToTeam) throw new ValidationError('Not invited to requested team');
