...
export class JobService {
  ...
  public async createJob(
    ...
    try {
      ...
      for (let task of tasks) {
        ...
        if (task.fromRoutes) {
          for (let i = 0; i < task.fromRoutes.length; i++) task.up_dep[task.fromRoutes[i][0]] = task.fromRoutes[i][1];
        }
  ...
  public async updateJob(
    ...
    if (!job) throw new MissingObjectError(`Job '${id}" not found with filter "${JSON.stringify(filter, null, 4)}'.`);
  ...
  async LaunchReadyJobs(
    ...
    try {
      ...
      if (_.isArray(queryNotStartedJobs) && queryNotStartedJobs.length > 0) {
        ...
        if (_.isArray(queryRunningJobs) && queryRunningJobs.length > 0) numRunningJobs = queryRunningJobs.length;

        /// Can't start more than maxInstances jobs
        let numJobsToStart = jobsToRun.length;
        if (jobDef.maxInstances) numJobsToStart = Math.min(jobDef.maxInstances - numRunningJobs, numJobsToStart);
