...
export class JobService {
    ...
    public async deleteJobs(
        ...
        if (_.isArray(jobsQuery) && jobsQuery.length === 0) {
            ...
            for (let i = 0; i < jobsQuery.length; i++) {
                ...
                let deleteStepOutcomeRes: any = await stepOutcomeService.deleteStepOutcome(
                    _teamId,
                    job._id,
                    correlationId
                );
                ...
                let deleteTaskOutcomeRes: any = await taskOutcomeService.deleteTaskOutcome(
                    _teamId,
                    job._id,
                    correlationId
                );
                ...
                if (deleted.acknowledged) {
                    ...
                    await rabbitMQPublisher.publish(_teamId, 'Job', correlationId, PayloadOperation.DELETE, {
                        id: job._id,
                    });
                }
            }
    ...
    public async createJobFromJobDefId(
        _teamId: mongodb.ObjectId,
        ...
        const jobDef = await JobDefModel.findOneAndUpdate(filterJobDef, { $inc: { lastRunId: 1 } }).select(
            'lastRunId name createdBy runtimeVars onJobTaskFailAlertEmail onJobCompleteAlertEmail onJobTaskInterruptedAlertEmail onJobTaskFailAlertSlackURL onJobCompleteAlertSlackURL onJobTaskInterruptedAlertSlackURL'
        );
        if (!jobDef) throw new MissingObjectError(`Job template '${_jobDefId}" not found`);
        await rabbitMQPublisher.publish(_teamId, 'JobDef', correlationId, PayloadOperation.UPDATE, {
            id: jobDef._id,
            lastRunId: jobDef.lastRunId,
        });

        return this.createJobFromJobDef(_teamId, jobDef, data, logger, amqp, responseFields);
    }

    public async createJobFromJobDefName(
        _teamId: mongodb.ObjectId,
        jobDefName: string,
        data: any,
        logger: BaseLogger,
        ...
        if (!jobDef) throw new MissingObjectError(`Job template '${jobDefName}" not found`);
        await rabbitMQPublisher.publish(_teamId, 'JobDef', correlationId, PayloadOperation.UPDATE, {
            id: jobDef._id,
            lastRunId: jobDef.lastRunId,
        });
    ...
    public async createJobFromJobDef(
        _teamId: mongodb.ObjectId,
        jobDef: any,
        data: any,
        logger: BaseLogger,
        ...
        let runtimeVars: any = {};
        if (jobDef.runtimeVars) runtimeVars = <any>jobDef.runtimeVars;

        if ('runtimeVars' in data) runtimeVars = Object.assign(runtimeVars, data.runtimeVars);

        let jobName = jobDef.name;
        if ('name' in data) jobName = data.name;

        let createdBy = jobDef.createdBy;
        if ('createdBy' in data) createdBy = data.createdBy;
        ...
        try {
            ...
            if (Object.keys(cd).length > 0)
                throw new ValidationError(
                    `Job contains a cyclic dependency with the following tasks: ${Object.keys(cd).filter(
                        (key) => cd[key]
                    )}`
                );
            ...
            for (let taskDef of taskDefs) {
                ...
                if (task.fromRoutes) {
                    for (let i = 0; i < task.fromRoutes.length; i++) {
                        ...
                        let rtCode = '';
                        if (task.fromRoutes[i][1]) rtCode = task.fromRoutes[i][1];
                ...
                for (let stepDef of await stepDefService.findAllStepDefs(_teamId, taskDef._id)) {
                    ...
                    const script: ScriptSchema = await scriptService.findScript(_teamId, stepDef._scriptId);
                    if (!script)
                        throw new MissingObjectError(
                            `Script "${stepDef._scriptId}" not found for task "${taskModel.name}"`
                        );
                    let step: StepSchema = {
                        _teamId: _teamId,
                        _jobId: newJob._id,
                    ...
                    if (taskModel.target == Enums.TaskDefTarget.AWS_LAMBDA) {
                        if (!step.lambdaZipfile) step.s3Bucket = config.get('S3_BUCKET_CUSTOMER_FUNCTIONS');
                        else step.s3Bucket = config.get('S3_BUCKET_TEAM_ARTIFACTS');
                        step.lambdaAWSRegion = config.get('AWS_REGION');
                    }

                    const stepModel: StepSchema = <StepSchema>(
            ...
            if (newJob) {
                await this.updateJob(
                    _teamId,
                    newJob._id,
                    { status: Enums.JobStatus.FAILED, error: err.message },
                    logger,
                    amqp
                );
            }
    ...
    public async createJob(
        ...
        try {
            ...
            if (!('runtimeVars' in job)) job.runtimeVars = {};
            ...
            for (let task of tasks) {
                ...
                for (let step of task.steps) {
                    ...
                    if (step.script.script_id) {
                        const script: ScriptSchema = await scriptService.findScript(_teamId, step.script._id);
                        if (!script)
                            throw new MissingObjectError(
                                `Script '${step.script._id}" not found for task "${taskModel.name}'`
                            );
    ...
    public async updateJob(
        ...
        if (job._jobDefId) {
            if (
                data.status == Enums.JobStatus.RUNNING ||
                data.status == Enums.JobStatus.COMPLETED ||
                data.status == Enums.JobStatus.SKIPPED
            ) {
                if (
                    job.status == Enums.JobStatus.INTERRUPTED ||
                    job.status == Enums.JobStatus.CANCELING ||
                    job.status == Enums.JobStatus.FAILED
                ) {
                    const failedJobs: JobSchema[] = <JobSchema[]>await this.findAllJobsInternal(
                        {
                            _teamId,
                            _jobDefId: job._jobDefId,
                            $or: [{ status: Enums.JobStatus.INTERRUPTED }, { status: Enums.JobStatus.FAILED }],
                        },
                        'id'
                    );
                    if (!failedJobs || (_.isArray(failedJobs) && failedJobs.length < 1)) {
                        try {
                            await jobDefService.updateJobDef(
                                _teamId,
                                job._jobDefId,
                                { status: Enums.JobDefStatus.RUNNING },
                                logger,
                                amqp,
    ...
    async LaunchReadyJobs(
        ...
        if (!jobDef) {
            ...
            if (!jobDefsExisting) throw new MissingObjectError(`Job template ${_jobDefId.toHexString()} not found.`);
                ...
                logger.LogError('Not launching job because job template launchingJobs property is true', {
                    _jobDefId: _jobDefId.toHexString(),
                });
            return;
        }
        ...
        try {
            ...
            const queryNotStartedJobs: JobSchema[] = await this.findAllJobsInternal({
                _jobDefId,
                _teamId,
                status: Enums.JobStatus.NOT_STARTED,
            });
            ...
            if (_.isArray(queryNotStartedJobs) && queryNotStartedJobs.length > 0) {
                    ...
                    if (_.isNumber(jobDef.misfireGraceTime) && job.dateScheduled) {
                        ...
                        if (lag <= jobDef.misfireGraceTime) {
                            ...
                            this.updateJob(
                                _teamId,
                                job._id,
                                { status: Enums.JobStatus.SKIPPED, error: 'Exceeded misfire grace time' },
                                logger,
                                amqp
                            );
                        }
                ...
                const queryRunningJobs: JobSchema[] = await this.findAllJobsInternal(
                    { _jobDefId: _jobDefId, _teamId, status: Enums.JobStatus.RUNNING },
                    'id'
                );
                ...
                if (numJobsToStart > 0 && jobDef.coalesce) {
                    for (let i = 0; i < jobsToRun.length - 1; i++) {
                        this.updateJob(
                            _teamId,
                            jobsToRun[i]._id,
                            { status: Enums.JobStatus.SKIPPED, error: 'Job skipped due to coalesce' },
                            logger,
                            amqp
                        );
                    }
                    ...
                    try {
                        await this.updateJob(
                            _teamId,
                            jobToRun._id,
                            { status: Enums.JobStatus.RUNNING, dateStarted: new Date().toISOString() },
                            logger,
                            amqp,
                            { status: Enums.JobStatus.NOT_STARTED }
                        );
                        ...
                        if (!(err instanceof MissingObjectError)) {
                            logger.LogError('Launching next available job', {
                                _jobDefId: jobToRun._jobDefId.toHexString(),
                                _jobId: jobToRun._id.toHexString(),
                            });
                        }
                    }
                    // if (resJobQuery) {
                    ...
                    for (let i = 0; i < numJobsToStart; i++) {
                        ...
                        try {
                            ...
                            if (!(err instanceof MissingObjectError)) {
                                logger.LogDebug('Launching next available job', {
                                    _jobDefId: _jobDefId.toHexString(),
                                    _jobId: jobToRun._id.toHexString(),
                                });
                            }
            ...
            try {
                ...
                let jobNotStarted: any = JobModel.find({
                    _teamId,
                    _jobDefId,
                    status: Enums.JobStatus.NOT_STARTED,
                }).limit(1);
    ...
    async CheckJobStatus(_teamId: mongodb.ObjectId, _jobId: mongodb.ObjectId, currentStatus: Enums.JobStatus) {
            ...
            _teamId,
            {
                ...
                $or: [
                    { status: null },
                    {
                        status: {
                            $in: [
                                Enums.TaskStatus.NOT_STARTED,
                                Enums.TaskStatus.WAITING_FOR_AGENT,
                                Enums.TaskStatus.PUBLISHED,
        ...
        if (_.isArray(tasks) && tasks.length > 0) {
            for (let task of tasks) {
                if (taskIds.indexOf(task._id.toHexString()) >= 0) continue;
                if (task.status) {
        ...
        if (!isJobComplete) {
            ...
            } else if (currentStatus === Enums.JobStatus.INTERRUPTING && taskStatuses[Enums.TaskStatus.RUNNING] === 0) {
                return Enums.JobStatus.INTERRUPTED;
            } else if (currentStatus === Enums.JobStatus.COMPLETED && taskStatuses[Enums.TaskStatus.RUNNING] > 0) {
                return Enums.JobStatus.RUNNING;
            } else if (currentStatus === Enums.JobStatus.INTERRUPTED && taskStatuses[Enums.TaskStatus.RUNNING] > 0) {
                return Enums.JobStatus.RUNNING;
            } else if (currentStatus === Enums.JobStatus.FAILED && taskStatuses[Enums.TaskStatus.RUNNING] > 0) {
                return Enums.JobStatus.RUNNING;
            }
    ...
    async UpdateJobStatus(
        ...
        if (!job) {
            if (!responseFields) responseFields = 'status';
            if (responseFields.indexOf('status"') < 0) responseFields += ' status';
            responseFields = responseFields.trim();
            job = await this.findJob(_teamId, _jobId, responseFields);
            if (!job)
                throw new MissingObjectError(
                    `Job '${_jobId.toHexString()}' not found for team '${_teamId.toHexString()}'`
                );
        }
        ...
        if (status != job.status) {
            ...
            if (jobUpdated) {
                job = jobUpdated;
                if (
                    jobUpdated.status >= Enums.JobStatus.COMPLETED &&
                    (jobUpdated.onJobCompleteAlertEmail || jobUpdated.onJobCompleteAlertSlackURL)
                ) {
    ...
    async LaunchTasksWithNoUpstreamDependencies(
        _teamId: mongodb.ObjectId,
        _jobId: mongodb.ObjectId,
        logger: BaseLogger,
        amqp: AMQPConnector
                        ...
                        try {
                            ...
                            if (err instanceof MissingObjectError) {
                                logger.LogWarning(err.message, {
                                    error: err,
                                    service: 'JobService',
                                    method: 'LaunchTasksWithNoUpstreamDependencies',
                                });
                            } else {
                                throw err;
                            }
        ...
        if (no_tasks_to_run) {
            logger.LogError(`No tasks to run`, {
                Class: 'JobService',
                Method: 'LaunchTasksWithNoUpstreamDependencies',
                _teamId,
                _jobId,
